...
- [Agentic Skill Decay: Which Capabilities Erode Under Agent Delegation](agentic-skill-decay.md) — Delegation removes the unprompted use of a capability rather than the knowledge, and the decay tracks which sub-tasks the agent absorbs and succeeds at quietly
...
- [Reading Copilot Feature Engagement by Its Threshold](copilot-feature-engagement-threshold.md) — The September 2026 feature engagement breakdown counts users who returned to each Copilot surface on a second day, a floor on repeat use rather than evidence of a habit
- [Anonymized Customization Metrics in the Copilot CLI](anonymized-customization-metrics-copilot-cli.md) — The September 2026 CLI customization fields count use and variety but withhold customer-defined names, so they cannot settle which of your customizations to retire
...
- [Judgment Relocation: Where Human Decisions Land in an Agent Factory](judgment-relocation.md) — Which five decisions keep a human owner once agents write the code, and the four conditions — capacity, comprehension, trustworthy signals, a return path — that separate a real owner from a nominal one
- [Agent Time Estimates Are Not Schedules](agent-time-estimates-not-schedules.md) — A coding agent's duration estimate reproduces the human timeline in its training data; rank scope with it, then bound the run with turn and tool-call caps and measure wall-clock on comparable past runs
- [The Citizen-Agent-Expert Operating Model for AI Coding](citizens-agents-experts-operating-model.md) — Where engineering value moves when AI writes most of the code: citizens originate, agents execute, and experts govern the guardrails that let generated code reach production
- [The First Edit Predicts Whether an AI Completion Survives](first-edit-predicts-completion-abandonment.md) — Across 53.6K in-IDE edits, customizing a completion first precedes abandoning it 23% of the time against 12% for functionality changes, with the conditions that bound the signal
