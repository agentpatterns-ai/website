...
AI coding assistants have decoupled code-writing from software-building — two activities fused so tightly for decades that separating them felt impossible. The data is measurable, and the displacement is visible. The identity crisis is underway. Whether AI amplifies or atrophies a career depends on a conscious choice: do you relocate your rigor, or let the tools erode it?
...
The Pragmatic Engineer's 2026 survey found that 95% of professional engineers use AI tools weekly. 56% report doing 70% or more of their engineering work with AI assistance. 55% regularly use AI agents: autonomous, multi-step tools beyond simple autocomplete ([Pragmatic Engineer 2026 AI tooling survey](https://newsletter.pragmaticengineer.com/p/ai-tooling-2026)).

This is baseline professional practice, not an early-adopter phenomenon.
...
Stack Overflow's 2025 Developer Survey found that 66% of developers report spending more time fixing "almost-right" AI-generated code. The time freed from boilerplate production has gone to review, debugging, and integration — the work that remains stubbornly human, not to leisure ([Stack Overflow 2025 Developer Survey](https://survey.stackoverflow.co/2025/ai)).
...
The identity shift would be manageable if practitioners could decide to become "builders" and move on. But there is a trap: the same tools that enable the transition also erode the skills needed to supervise them.
...
Martin Fowler, drawing on Chad Fowler's framing, offers what may be the most useful mental model for navigating this transition: rigor relocation. Engineering discipline moves in an AI-augmented workflow instead of vanishing ([Fowler: Harness Engineering](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html)).
...
The abundance created by AI coding assistants produces counterintuitive problems: more code, longer reviews, higher churn, and threatened open-source communities.

Huntley identifies one of the sharpest: developers increasingly bypass dependency management entirely, generating solutions on demand rather than pulling in maintained libraries. Why add a dependency with its maintenance burden, security surface, and recurring upgrades when an agent can generate the equivalent functionality inline? ([Huntley: what is the point of libraries?](https://ghuntley.com/libraries/))
...
The broader abundance paradox: when code is cheap to produce, the skills that become expensive are the ones that keep you from being overwhelmed by it. Curation, review, architectural coherence, and the judgment to know when not to generate more code become the scarce resources.
...
It is a structural change in how the profession onboards new practitioners, not merely a hiring cycle. The traditional pipeline — junior developer writes boilerplate, learns patterns through repetition, gradually takes on design responsibilities — assumed that someone needed to write that boilerplate. When agents handle it, the entry-level role disappears.
...
The honest assessment: this transition is a genuine loss for some and a genuine opportunity for others, and the deciding factor is whether practitioners consciously develop the skills that AI cannot replicate — see the [skill atrophy](skill-atrophy.md) mechanism above for what happens when they do not.
...
Huntley's summary is blunt: "Execution is now cheap. All that matters now is brand, distribution, ideas and retaining people who get it" ([Huntley: execution is cheap](https://ghuntley.com/dothings/)). This overstates the case — execution quality still matters, and someone has to ensure it — but the directional claim is correct. What creates value has shifted.

## The choice

The split between coder and builder is a choice you make repeatedly in how you engage with these tools, not something that happens to you. Reading generated code before accepting it, designing a constraint system instead of skipping one, and forming a hypothesis before asking the AI to debug are the daily habits that separate supervision from atrophy; reflexively generating more code instead of questioning whether it is needed feeds the abundance problem this page opened with (GitClear's 26% churn increase, cited above).

The profession is differentiating, not dying. The deciding question is what kind of engineer you want to be, and the deliberate work required to become that person — not which model or agent framework to adopt.
...
- [Geoffrey Huntley: Execution Is Cheap](https://ghuntley.com/dothings/) — competitive advantage relocation
