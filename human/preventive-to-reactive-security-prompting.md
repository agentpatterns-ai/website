...
- **Teams with rigorous SAST/CI gates may see redundant ceremony.** Snyk, Semgrep, or CodeQL on every PR catch the same vulnerability classes mechanically. Front-loading still helps by reducing the number of findings the gates surface — but the marginal value drops.
...
- Front-loading is not a security guarantee — see [Prompt as Security Knob](../anti-patterns/prompt-as-security-knob.md); independent verification of deployed paths remains required
...
- [Prompt as Security Knob](../anti-patterns/prompt-as-security-knob.md) — why front-loading shifts the distribution but never certifies a single output as secure
