...
The agent runs one accumulating conversation across many turns against a stored objective. After each turn, the harness injects a templated message that re-states the objective, reports the remaining budget, and demands a completion audit before the agent can mark the goal done. A second template (`budget_limit.md`) fires at the budget cap to wind the agent down. There are two stop conditions: the agent calls a "goal complete" tool, or the budget fires.

This pattern differs from a [Ralph Wiggum loop](ralph-wiggum-loop.md), which runs each iteration in a fresh context window with state on disk. Goal-driven loops keep one session and steer it through structured turn-end injection.
