...
GitHub Copilot's review is the implicit binary form: in [29% of reviews the agent stays silent and in 71% it surfaces actionable feedback](https://github.blog/ai-and-ml/github-copilot/60-million-copilot-code-reviews-and-counting/). A multi-level dial generalises it — silence is the lowest rung, deep agentic exploration the highest.
...
Holding resolution rate constant across levels is the calibration commitment: High costs more but does not flood with low-quality flags. Both axes matter — resolution rate gates whether the higher-effort flags are useful at all.

## Routing Axes

The dial composes with structured routing. Four axes:
...
The cost case for High is that high-effort review on critical paths is cheaper than post-merge remediation. Cursor's [per-run pricing is $1.00-$1.50 by PR size](https://cursor.com/blog/may-2026-bugbot-changes), and effort levels require usage-based billing.

The arithmetic only holds when routing concentrates High on high-stakes runs. A reviewer toggling High on every PR pays the premium and accumulates the alert-fatigue cost the dial existed to avoid.

## When This Backfires

- **Resolution rate is not precision.** It counts both "developer fixed it" and "developer made it go away." The 35%-more-bugs claim at High does not rule out proportionally more false positives that authors muted. Teams needing a precision floor ([security under 3% FPR, style under 2%](https://www.codeant.ai/blogs/ai-code-review-false-positives)) measure noise themselves.
- **High-effort default creates alert fatigue.** [Signal over volume](signal-over-volume-in-ai-review.md) is the dominant trust factor. Pinning High at the policy level burns reviewer attention on routine PRs and un-funds the high-stakes runs the dial exists to protect.
- **Routing policy drift in Custom.** Natural-language Custom is an instruction file with the same [primacy and drift issues](../instructions/system-prompt-altitude.md). A correct policy in May 2026 may misroute six months later. No eval is exposed by default.
- **Cost-blind defaults.** Without per-PR cost ranges, reviewers toggle High on everything. Pinning High at the team level collapses the dial back to a fixed-pipeline calibration.
...
Tunable effort assumes a calibrated, published curve and a routing policy that concentrates the higher tier on the runs that pay for it. Without both, the dial reduces to per-PR cost optionality with no signal benefit.
...
A 30-line bug fix in `tests/integration/` from a long-tenured engineer runs at Default — Bugbot finds an expected 0.7 bugs/run, the review completes fast, and the reviewer trusts that silence on style is real silence. A 1,200-line refactor touching `src/auth/session.ts` from a first-time contributor runs at High — Bugbot spends more reasoning time on the diff, surfaces 0.95 bugs/run on average, and the reviewer reads each flag knowing the routing logic concentrated effort here on purpose.

Pinning Default at the team level and letting the Custom policy escalate is the inversion of biasing-up: it preserves the silence-as-output contract for routine work while making the higher tier opt-in by policy rather than per-PR clicking.
