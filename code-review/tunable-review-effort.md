...
Cursor's Bugbot externalized this dial on [2026-05-11](https://cursor.com/changelog/05-11-26):
...
- File-path criticality — auth, payment, or crypto paths route to High. This is the same axis [tiered code review](tiered-code-review.md) uses for human escalation.
- Change size — diffs above an LOC threshold route to High.
- Author trust — first-time contributors or external forks route to High. This is the same signal `CODEOWNERS` already encodes.
- Historical defect rate — directories with elevated post-merge bug rates route to High. Cursor exposes no signal, so encode it in Custom.
...
High-effort review on critical paths is cheaper than post-merge remediation. Cursor's [per-run pricing is $1.00 to $1.50 by PR size](https://cursor.com/blog/may-2026-bugbot-changes), and effort levels require usage-based billing.
