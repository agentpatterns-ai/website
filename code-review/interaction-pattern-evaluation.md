...
> Merge and reject labels misclassify agentic PR quality: most rejections are not failures, some merges hide no review. Evaluate by interaction pattern, not outcome.

Outcome labels conflate three signals: whether the agent's output was viable, whether reviewers engaged, and whether the interaction was observable. An analysis of 11,048 closed agentic PRs (717 manually inspected) found 31.2% of rejections driven by workflow constraints (duplicates, abandonment, policy mismatches). Another 33.1% had no observable rationale. Among merges, 15.4% needed explicit reviewer involvement and 5.5% closed without visible interaction ([MSR 2026 mining challenge entry](https://2026.msrconf.org/details/msr-2026-mining-challenge/15/Why-Are-Agentic-Pull-Requests-Merged-or-Rejected-An-Empirical-Study)). Interaction-pattern evaluation replaces the binary outcome with a structured signal set from the review trace itself.

## What to measure

Five signals separate agent capability from workflow noise:

- Review-loop completion — did the PR get at least one substantive review and converge in bounded rounds? Reviewer engagement is the strongest single merge predictor in a regression on 33,596 agent-authored PRs ([arXiv:2602.19441](https://arxiv.org/abs/2602.19441v1)).
...
Task type confounds aggregate merge rate on its own. Documentation accepts at 82.1% against 66.1% for new features, a 16-point gap that exceeds inter-agent variance on most categories ([arXiv:2602.08915](https://arxiv.org/abs/2602.08915v2)). Stratify every metric above by task type before you compare agents.

## Why it works

The mechanism is causal pathway separation. An outcome label collapses three independent processes (output viability, reviewer engagement, and trace observability) into one binary. Any comparison built on it carries noise from the other two. The MSR 2026 inspection shows that roughly two-thirds of rejection labels and one-fifth of merge labels carry information unrelated to capability. Interaction signals (review-loop, reviewer-commit, and force-push counts, plus the abandonment timestamp) come out separately, so a metric built on them recovers a cleaner capability estimate. The pattern recurs in Alam et al.'s 8,106 fix-related PRs, where test failures and prior resolution drove non-integration rather than capability defects ([arXiv:2602.00164](https://arxiv.org/abs/2602.00164v1)).
...
- Homogeneous task mix and stable reviewers — if an agent only ships documentation PRs to one team, the rationale-loss bucket collapses and merge rate is an acceptable proxy. The 82.1% documentation baseline is high enough that workflow noise matters less ([arXiv:2602.08915](https://arxiv.org/abs/2602.08915v2)).
- Curated, high-engagement populations — Watanabe et al. found 567 Claude Code PRs across 157 maintained OSS projects merged at 83.8%, with 54.9% unmodified ([arXiv:2509.14745](https://arxiv.org/abs/2509.14745v3)). When abandonment is rare by selection, outcome rates carry more signal.
...
A platform team evaluates four agents over a quarter and reads off aggregate merge rates: Codex 82.6%, Cursor 65.2%, Devin 53.8%, Copilot 43.0% ([arXiv:2602.19441](https://arxiv.org/abs/2602.19441v1); Cursor rate from [arXiv:2602.17084](https://arxiv.org/abs/2602.17084v1)). The headline ranking is Codex > Cursor > Devin > Copilot.

Stratifying by interaction pattern surfaces a different story. Copilot and Devin PRs concentrate in reviewer-mediated workflows. Their merges include high rates of reviewer commits and review-loop completion, both positive capability signals once isolated. Codex and Cursor PRs merge with minimal interaction, which the outcome metric rewards but which also obscures whether those merges include the 5.5% "no visible trace" bucket where review was effectively absent ([MSR 2026 entry](https://2026.msrconf.org/details/msr-2026-mining-challenge/15/Why-Are-Agentic-Pull-Requests-Merged-or-Rejected-An-Empirical-Study)). Task stratification compounds the correction. Copilot ships proportionally more bug-fix PRs than humans do (42.2% of its mix versus 26.9%), and bug-fix has a structurally lower acceptance rate than documentation. The headline gap shrinks further as a result ([arXiv:2507.15003](https://arxiv.org/abs/2507.15003v1)). The team picks a tier-1 agent based on per-task, per-interaction-mode performance rather than the aggregate.
...
- Trust the interaction signals over the label itself: the 717-case manual inspection found two-thirds of rejection labels and one-fifth of merge labels carry no capability information ([MSR 2026 entry](https://2026.msrconf.org/details/msr-2026-mining-challenge/15/Why-Are-Agentic-Pull-Requests-Merged-or-Rejected-An-Empirical-Study))
...
- [arXiv:2602.19441](https://arxiv.org/abs/2602.19441v1) — Nachuma & Zibran (MSR 2026): logistic regression on 33,596 agent-authored PRs; reviewer engagement strongest positive predictor, force pushes strongest negative
- [arXiv:2602.00164](https://arxiv.org/abs/2602.00164v1) — Alam et al.: 8,106 fix-related agentic PRs; 12 failure reasons dominated by duplicates and CI failures
- [arXiv:2602.08915](https://arxiv.org/abs/2602.08915v2) — Pinna et al.: task-stratified analysis of 7,156 PRs; 16-point documentation-vs-features acceptance gap
- [arXiv:2509.14745](https://arxiv.org/abs/2509.14745v3) — Watanabe et al.: 567 Claude Code PRs across 157 maintained OSS projects; 83.8% merge baseline counter-evidence
- [arXiv:2507.15003](https://arxiv.org/abs/2507.15003v1) — Li, Zhang & Hassan: AIDev dataset of 456K agent-authored PRs; per-agent task mix
- [arXiv:2602.17084](https://arxiv.org/abs/2602.17084v1) — PR description style study across 5 AI coding agents; per-agent merge rates including Cursor (65.2%)
