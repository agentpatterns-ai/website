...
- [Per-Reviewer Context Views for Code Review Agents](per-reviewer-context-views.md) — A context builder composes a dimension-scoped context prompt for each specialized review agent from repository structure and a dependency graph, instead of handing every reviewer the same diff
...
- [Agent Approval Authority in Code Review](agent-approval-authority.md) — Once an admin enables it, an AI reviewer's approval satisfies the required-approvals rule; the two conditions that decide where the human gate belongs
- [Agent-Resolved Review Threads](agent-resolved-review-threads.md) — A reviewing agent that closes its own threads costs you nothing unless you gated merge on conversation resolution or measured precision from resolve clicks
- [Configuring the Code Review Request Surface](review-request-surface.md) — Whether an agent review runs is a branch ruleset, how deeply it reads is a per-scope dropdown, and whether it can approve is a policy with a file-glob list; three knobs that scope by different things
