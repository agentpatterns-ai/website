...
The standard MCP pattern registers every tool definition at session start. As tool collections grow — hundreds of tools across dozens of servers — this becomes a large context cost. [Anthropic's MCP code execution research](https://www.anthropic.com/engineering/code-execution-with-mcp) reports that large tool collections can consume 150,000+ tokens on tool definitions alone before any task work begins.
...
Include a manifest. A top-level `README.md` or `manifest.json` that lists available server categories and their purposes helps the agent orient without reading every subdirectory.
...
Keep file content minimal. Each tool file should contain the function signature, a brief description, and the parameters. The purpose is selective loading, not documentation.
