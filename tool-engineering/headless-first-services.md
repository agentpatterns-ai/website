...
The first wave of public APIs, around 2010, targeted third-party developers and mostly retreated behind walls. The second wave lets agents act on behalf of the authenticated user, with the same permissions and data visibility as that user inside the first-party app ([Brandur Leach, 2026](https://brandur.org/second-wave-api-first)).
...
Gaps break agent workflows. When a flow needs a browser session — a widget-only upload, a dialog approval, a confirmation email click — the flow pulls the user back mid-task. Partial coverage creates false confidence that an agent can finish a job end-to-end.
...
- Rate limits sized for one user's agent — Slack's MCP server caps at 50 channel reads and 100 profile reads per minute: too little for a scraper, plenty for one assistant ([Slack MCP docs](https://docs.slack.dev/ai/slack-mcp-server/#rate-limits)).

For MCP, follow the [MCP server design](mcp-server-design.md) checklist. For CLI, apply [token-efficient tool design](../token-engineering/token-efficient-tool-design.md) and ship `--json`, distinct exit codes, and non-interactive defaults.
