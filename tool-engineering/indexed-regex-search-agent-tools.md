...
- Multi-agent fleets sharing a workspace, where every agent pays the search cost and a shared index amortizes the scan.
...
- Claude Code's documented evaluation of agentic search versus indexed retrieval is the strongest counter-evidence that indexing is not universally the right call.
