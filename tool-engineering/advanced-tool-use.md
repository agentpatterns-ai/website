...
Learn it hands-on with the [Tool Discoverability at Scale guided lesson](https://learn.agentpatterns.ai/tool-engineering/tool-discoverability-at-scale/), which includes quizzes.
...
A typical multi-server MCP setup (GitHub, Slack, Sentry, Grafana, Splunk) can use about 55K tokens in tool definitions alone, before any actual work. At scale, tool definitions become the dominant cost, not conversation content. For design-level strategies, see [Token-Efficient Tool Design](../token-engineering/token-efficient-tool-design.md) and [Filesystem-Based Tool Discovery](filesystem-tool-discovery.md) ([source](https://www.anthropic.com/engineering/advanced-tool-use)).
...
Token impact: ~55K → ~8.7K, an 85%+ reduction ([source](https://www.anthropic.com/engineering/advanced-tool-use)). The model loads only 3–5 tools per request instead of 50+.
...
MCP toolset support lets you defer entire servers while keeping high-use tools loaded:
...
Token impact: a 37% reduction (43,588 → 27,297 tokens on complex multi-step research tasks) ([source](https://www.anthropic.com/engineering/advanced-tool-use)).

Latency impact: it removes 19+ inference passes by orchestrating 20+ tool calls in a single code block ([source](https://www.anthropic.com/engineering/advanced-tool-use)).
...
- Parameter correlations: critical bugs get full escalation objects; feature requests do not
- Progressive complexity: minimal tickets exist, so not everything needs every field
...
- [Token-Efficient Tool Design](../token-engineering/token-efficient-tool-design.md) — reducing per-tool output tokens
