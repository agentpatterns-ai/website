...
- [Model-Switch Lifecycle Hooks: Gating a Mid-Session Model Change](model-switch-lifecycle-hooks.md) — `PreModelSwitch` and `PostModelSwitch` expose a mid-session model change as a blockable event, with the re-cache cost estimate supplied before the switch happens
...
- [Terminal-First Agent Interfaces with Browser Escalation](terminal-first-browser-escalation.md) — Default an enterprise agent to a terminal calling platform APIs and escalate to a browser only for session-bound, render-bound, and UI-authored tasks
...
- [Choosing an Agent Tool Interface: Shell or Typed Catalog](agent-tool-interface-selection.md) — Where execution can be isolated a shell outscores a typed catalog on enterprise work by 6 to 25 pp, and layering the catalog back on top recovers nothing
