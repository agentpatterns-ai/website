...
- [Event-Driven Agent Routing](../patterns/agent-design/event-driven-agent-routing.md) — the fresh-session-per-event alternative when warm context is not worth keeping
- [Agent Event Streaming](../patterns/agent-design/agent-event-streaming-model.md) — runtime-side typed event vocabulary; channels are an inbound source that feeds it
