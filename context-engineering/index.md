...
- [Extended Prompt Cache TTL for Long Agent Sessions](extended-prompt-cache-ttl.md) — Switch to Anthropic's 1-hour cache TTL when interactive coding sessions sit idle longer than 5 minutes; the 2x write premium pays back on the first re-use past the default TTL
...
- [PEEK: Orientation Cache for Recurring-Context Agents](peek-orientation-cache.md) — A small, constant-sized prompt artifact that caches reusable orientation knowledge about a recurring large context — distinct from trajectory replay and playbook strategy memory, conditional on stable context and a working invalidation surface
