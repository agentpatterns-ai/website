...
Overshooting is the more expensive error. At Anthropic's 300-second lifetime, a 900-second interval means every ping lands on a dead entry and pays the full write price, so the keepalive "costs 4x more than never pinging" ([arXiv:2607.19214](https://arxiv.org/abs/2607.19214v2)). Measure your own retention rather than copying a table, because cross-provider lifetimes move and they move per model. On GPT-5.5 and earlier, OpenAI defaults organizations without zero data retention to a 24-hour window, far longer than the eviction-on-idle behavior usually assumed. On GPT-5.6 and later the same provider went the other way: `prompt_cache_options.ttl` accepts only `"30m"`, and a prefix stays eligible "at least 30 minutes after the latest write or reuse" ([OpenAI prompt caching guide](https://developers.openai.com/api/docs/guides/prompt-caching)). A 30-minute window with free refresh-on-read leaves a keepalive very little to buy.
