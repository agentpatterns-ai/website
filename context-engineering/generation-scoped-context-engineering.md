...
- Examples to interface design. For tool usage, "giving examples actually constrains them to a certain exploration space." Design expressive parameters instead — a Todo status modelled as an enum of `pending`, `in_progress`, `completed` tells the model how to use the tool without boxing it in.
...
- The mechanism is a reconciliation tax: a guardrail the model no longer needs becomes a conflicting directive it must resolve before acting, crowding out context it can now read directly.
