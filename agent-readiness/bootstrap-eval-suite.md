...
## Step 8b — Multiverse Reporting and Human Verification

Single-run, single-model eval results carry hidden methodological variance. Two safeguards before treating results as ground truth:

- **Multiverse runs across model families.** Each case should run against at least two model families (e.g., one Sonnet generation + one Opus generation, or one Anthropic + one OpenAI) and report the distribution, not a point estimate. Single-family runs can mask family-specific failure modes that surface as soon as the production model is rotated. Rule from [`nonstandard-errors-ai-agents`](../verification/nonstandard-errors-ai-agents.md) §Multiverse Analysis. In `evals/run.sh`, parameterize the model id and emit per-model rows; in CI, run the matrix across the chosen families.
- **Human-authored seed cases.** Cases generated by an LLM without human verification create feedback loops that degrade across generations. Mark every case file with `authored_by: human|llm-assisted|llm-only` and `verified_by: <name|null>`. The runner refuses to count `llm-only` cases that are not `verified_by`-stamped toward the P0 pass-rate gate. Rule from [`synthetic-ground-truth-fallacy`](../fallacies/synthetic-ground-truth-fallacy.md).

Both are additive — apply on top of the base scaffold once the suite has a steady stream of cases.
