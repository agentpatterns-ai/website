...
Compared to stopping and restarting, it keeps the context the agent has built: files read, decisions made, progress artifacts. Compared to asking inline without framing, the tagged markers signal that the question is bounded. They cue the agent to answer narrowly and resume the prior goal, rather than treat the question as a new implicit objective.

The causal chain is well-grounded:

- Summarization favors high-frequency content and drops one-off constraints ([LangChain, "Context management for deep agents"](https://blog.langchain.com/context-management-for-deepagents/)) — an untagged mid-task question is indistinguishable from a new instruction
