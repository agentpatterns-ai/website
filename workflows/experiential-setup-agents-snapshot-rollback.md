...
[Repo2Run (Hu et al., 2025)](https://arxiv.org/abs/2502.13681) independently validates this mechanism for Python-repo Dockerfile generation. It documents "environment pollution" as a distinct failure mode (for example, `pip install tensorflow-gpu` failing but installing `python-version`), and its atomic `docker commit`/rollback design hit 86.0% against 6.0%/9.0%/22.1% for `pipreqs`, `SWE-agent`, and a README-only LLM baseline. SetupX layers cross-repo experience reuse on top of this same primitive.
