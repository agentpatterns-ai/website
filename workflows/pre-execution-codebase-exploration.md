...
Phase 1: context scoping. The layer runs a targeted analysis of codebase structure and constraints. It identifies the relevant modules, maps the dependencies, and sets the scope of the change. The goal stays lightweight: enough to inform the synthesis, not a full exploration.

Phase 2: multi-perspective analysis. The layer examines potential fix paths and exploration opportunities from several angles. This surfaces ambiguities and shows which reproduction steps or expected behaviors are missing from the original statement.

Phase 3: synthesis. The layer produces an enhanced problem statement that adds reproduction steps, expected behaviors, and targeted exploration hints. It does not replace the original statement. It adds what the agent would otherwise have to discover, or fail to discover, on its own.
...
Developer-driven exploration (interactive mode). Claude Code's [Plan Mode](../tools/claude/plan-mode.md) enforces the same principle interactively. A read-only exploration phase surfaces the agent's understanding before it makes any changes. The agent reads the codebase, asks clarifying questions, and produces a plan. You review the plan before execution begins. ([Claude Code best practices](https://code.claude.com/docs/en/best-practices))

The "let Claude interview you" pattern extends this. A minimal prompt triggers the agent to interview you with structured questions. The agent then synthesizes the findings into a spec file before a clean implementation session begins. ([Claude Code best practices](https://code.claude.com/docs/en/best-practices))

CLAUDE.md as persistent pre-loading. Codified codebase knowledge in CLAUDE.md pre-loads context for every session without per-task manual expansion. This is the persistent equivalent: invest once in codebase documentation, then spread the benefit across every agent invocation. ([Anthropic: Effective Context Engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents))

## What to add to the problem statement

Before you launch an agent on a task, add:

- Reproduction steps — the exact sequence that demonstrates the current broken behavior
- Expected behaviors — what correct behavior looks like, not just what the bug is
...
On SWE-bench Verified, CodeScout achieves a 20% improvement in resolution rates and resolves 27 additional issues compared to baseline. The gain comes from better task framing, not improved agent capability. ([arXiv:2603.05744](https://arxiv.org/abs/2603.05744v2))
...
The two prompts below show a vague task that causes over-exploration versus an enhanced version that adds the three elements the page identifies: reproduction steps, expected behavior, and exploration hints.
