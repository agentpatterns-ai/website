...
- **Import-graph enforcement**: a CI rule fails the build on imports from `prototypes/` into any production path. Without this rule, the disposable artefact silently becomes the production system through one accidental import.
