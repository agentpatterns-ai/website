...
Related lesson: [Research, Plan, Implement](https://learn.agentpatterns.ai/workflows/research-plan-implement/) — a hands-on lesson with quizzes covers this concept.
...
Human-led. Formalize the feature requirements based on what the Research and Prototype phases revealed. The PRD is a structured Markdown document — the same externalized-spec role [spec-driven development](spec-driven-development.md) formalizes — capturing who uses the feature, what behavior it produces, and what the success criteria are.
...
This decomposition step is the highest-value handoff point in the model. Issue description quality is the main factor in delegation success — specific context, acceptance criteria, and file references directly affect output quality ([Issue-to-PR Delegation Pipeline](issue-to-pr-delegation-pipeline.md)).
