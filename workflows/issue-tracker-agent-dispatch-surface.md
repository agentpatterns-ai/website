...
| Status-echo gating | Every progress update posted as a new comment buries the original requirements; the `hide-older-comments: true` knob in `gh-aw` exists because the spam pattern was severe enough to standardise. ([github.github.com gh-aw](https://github.github.com/gh-aw/reference/safe-outputs/)) |
...
- **Vendor field-model mismatches** — Atlassian community reports describe Rovo agents that "hallucinate success on write actions" and "don't distinguish cleanly between field types and their scope," meaning the status-echo contract appears to work but actually fails silently. When one user has access to multiple boards, Rovo picks the wrong board's context, breaking exactly at organisational scale. ([Atlassian Community: Rovo limitations](https://community.atlassian.com/forums/Rovo-questions/Does-anyone-else-feel-like-Rovo-is-just-A-not-AI/qaq-p/3215367))
