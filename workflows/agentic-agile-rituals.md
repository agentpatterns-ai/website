...
- **Daily stand-ups as the synchronisation primitive** — when the agent commits every minute, synchronisation happens in CI and dashboards, not in a meeting. The remaining human ceremony is the [trace retro](../observability/traces-need-feedback-to-power-learning.md), not the standup.
...
- **Solo developer or single-agent context** — sprint planning and demo cadence add friction with no synchronisation payoff when one developer iterates in minutes; stay on [failure-driven iteration](failure-driven-iteration.md) instead. Continuous flow plus automated trace diagnostics is the correct ceiling.
