...
Local plan mode binds three things that have different optimal runtimes: where the plan is *generated*, *reviewed*, and *executed*. [Anthropic's ultraplan documentation](https://code.claude.com/docs/en/ultraplan) names "Targeted feedback", "Hands-off drafting", and "Flexible execution" as the three value props — each maps to one of those bindings.
...
The teaching is the *deferral*: the runtime choice belongs at approval, not at dispatch.
...
When the status changes to `◆ ultraplan ready`, the session link opens a dedicated review view on claude.ai. The surface gives three review affordances ([ultraplan docs](https://code.claude.com/docs/en/ultraplan)):
...
Asking Claude to address the comments produces a revised draft. Iterate as many times as needed before choosing where to execute.
...
**Execute on the web** — select **Approve Claude's plan and start coding** and Claude implements the plan in the same cloud session. The terminal status indicator clears. When the implementation finishes, the diff is reviewable in the web interface and a PR is opened from there ([Claude Code on the web docs](https://code.claude.com/docs/en/claude-code-on-the-web)).

**Teleport back to the terminal** — select **Approve plan and teleport back to terminal**. This option appears only when the session was launched from the CLI *and* the terminal is still polling. The cloud session is archived so it does not continue working in parallel ([ultraplan docs](https://code.claude.com/docs/en/ultraplan)).
...
The **Cancel** option is the failure-mode fallback: if the polling terminal closed or the developer wants to defer execution entirely, the plan still survives as a versioned artifact.
...
The triad as a *workflow* — async cloud plan + structured review + execute-anywhere — is currently shipped end-to-end only by [Claude Code's ultraplan](https://code.claude.com/docs/en/ultraplan).

Adjacent capabilities elsewhere cover individual layers:

- **GitHub Copilot's coding agent + Copilot CLI**: cloud execution and one-way [cloud-to-local handoff](cloud-local-agent-handoff.md) via "Continue in Copilot CLI", but the plan itself is not the reviewable artifact — the PR is. There is no inline-comment review surface on the plan.
