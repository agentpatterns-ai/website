...
- [Discovery-Only Refactor Pass: Surface Candidates Before Touching Code](discovery-only-refactor-pass.md) — A read-only skill that ranks deepening opportunities against Ousterhout's deep-module vocabulary and stops at a candidate list — the human picks one, then a separate session does the work
...
- [Agent-Led Dev-Environment Iteration with Validation and Rollback](agent-led-dev-environment.md) — Agent authors its own Dockerfile, gated by a smoke test, with snapshot rollback per attempt and an audit log per change — the next shape after operator-authored environment bootstrapping
- [Distilled Bootstrap Contract: Agent-Authored Repo Setup](distilled-bootstrap-contract.md) — Run a multi-agent discovery pipeline once, then version the resolved environment-setup heuristics as a persistent contract that downstream agents consume instead of rediscovering setup
...
- [Code Cleanliness as an Agent Cost Lever](code-cleanliness-agent-cost-lever.md) — Cleaner code does not change agent pass rate but cuts token use 7-8% and file revisitations 34% in a controlled minimal-pair study — operational cost lever, not a capability fix
...
- [Programmatic Cloud-Agent Dispatch via REST API and Webhooks](programmatic-cloud-agent-dispatch.md) — Dispatch coding agents from REST endpoints, webhooks, and cron — safe only with caller-side dedupe, payload sanitisation, per-trigger budget caps, and out-of-band principal logging
- [Multi-Repo and No-Repo Coding Agent Automation Templates](multi-repo-no-repo-automation-templates.md) — Decouple trigger from scope only when the vendor exposes the primitive and the four implicit safeguards (dedupe, signal verification, credential scoping, audit attribution) are reconstructed at the automation level
- [Issue-Tracker as Agent Dispatch Surface](issue-tracker-agent-dispatch-surface.md) — Assign a coding agent from Jira, Linear, or GitHub Issues — the ticket becomes the prompt; viable only with WRAP-style ticket discipline, assignment-vs-mention conventions, and status-echo gating
...
- [Documentation-Guided Legacy Migration](documentation-guided-legacy-migration.md) — Convert a legacy C repository into architecture-aware documentation, feed that documentation to coding agents as the migration blueprint, then validate by re-documenting the translated output and diffing against the original
- [One-Click CI Auto-Fix: Human-Triggered Cloud-Agent Remediation for Failing GitHub Actions](one-click-ci-auto-fix.md) — A bounded-autonomy pattern where a human click hands a failing GitHub Actions run to a cloud agent that pushes a fix to the existing branch and tags the operator for review
- [Semantic Issue Search from Chat vs Query Syntax](semantic-issue-search-natural-language.md) — Use natural-language issue search for symptom-to-issue lookup and duplicate detection; fall back to keyword or Boolean filters for exact identifiers, audit queries, and live incident triage
