...
Prioritise by `ET/run × runs/day`, not `ET/run`. The published cuts on incremental indexing (70–80% projected) and CI Failure Doctor deduplication (40–60% projected) ranked highest precisely because both workflows ran many times a day ([gh-aw Discussion #19197](https://github.com/github/gh-aw/discussions/19197)).
...
- **Configuration repair.** One GitHub workflow hit a 64-turn fallback loop because bash patterns blocked the tool it needed ([GitHub Blog](https://github.blog/ai-and-ml/github-copilot/improving-token-efficiency-in-github-agentic-workflows/)). Misconfiguration shows up in the auditor as anomalously high per-run cost — investigate before optimising the average.
