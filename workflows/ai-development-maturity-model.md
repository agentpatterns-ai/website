...
You do not believe AI tools produce useful output. You may have tried an early or poorly configured tool and dismissed it. Usage is minimal or zero. The risk here is missing productivity gains that [compound over time](compound-engineering.md).

The exit condition: a task where the agent saves you meaningful time or produces output you could not produce as quickly alone.

### Phase 2: First experiments

You use the tool occasionally, on low-stakes tasks. Output quality seems inconsistent. There is no [systematic approach to its context](../context-engineering/seeding-agent-context.md) — just occasional invocation and occasional surprise.

The trap: concluding that inconsistency reflects the tool's ceiling rather than the quality of your input (prompt, context, task specification). Underspecified prompts and missing context drive most inconsistency at this phase.
...
You have seen enough failures to be cautious but have not yet developed [systematic verification habits](../verification/tdd-agent-development.md). Usage contracts. You revert to manual work for "important" tasks.

The trap: [over-generalizing from failures](failure-driven-iteration.md) to conclude the tool is not useful, rather than identifying which task types and contexts produce reliable output.
...
You design workflows around agent capabilities from the start. Multi-step pipelines, parallel agent execution, agent-generated pull requests, automated review — the agent is a participant in your development process, not a tool you invoke occasionally.
...
Trust does not increase steadily. Most developers pass through a trough at Phase 4 before reaching calibrated use.

## Team dynamics

Teams rarely move through phases in sync. Your team may have members at Phase 2 and Phase 6 at the same time. This creates friction: the Phase 6 member builds infrastructure the Phase 2 member does not use; the Phase 2 member reviews agent output without the verification habits the Phase 6 member takes for granted.

Surfacing the model helps teams find gaps and set baseline expectations. Not everyone needs to reach Phase 7, but Phase 5 (calibrated use with verification habits) is a reasonable minimum for teams working in agent-assisted codebases.
...
The plateau (stuck at Phase 5) reaches calibrated use but does not invest in context engineering. The developer keeps working prompt-by-prompt rather than building infrastructure that compounds.
...
Month 2 — Phase 4: the production bugs erode trust. They revert to writing code by hand for "important" tasks, using the agent only for low-stakes refactoring. Usage drops by 70%.
...
Month 6 — Phase 6: they add an `AGENTS.md` to the repo documenting conventions, a `.claude/commands/` directory with reusable task templates, and a seed file that gives agents access to the domain model at runtime. Per-task prompt quality matters less; the infrastructure carries context by default.
...
The key observation: Phase 4 feels like the tool does not work. It does — the inputs were wrong. The exit is systematic verification, not abandonment.
