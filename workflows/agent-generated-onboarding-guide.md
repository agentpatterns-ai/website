---
title: "Agent-Generated Onboarding Guide as a Durable Artifact"
description: "Use an agent to synthesize a teammate ramp-up guide from the repository, then version-control the artifact and regenerate on architectural change rather than writing onboarding docs by hand."
...
> An agent synthesizes a teammate onboarding guide from the repository; you version-control that artifact and regenerate it on architectural change rather than on a calendar.

## Artifact, not conversation

An agent that reads a codebase can answer onboarding questions live — see [Agent-Powered Codebase Q&A and Onboarding](codebase-qa-onboarding.md). The artifact variant is the complement. A live Q&A session disappears when the newcomer closes the terminal. The artifact is instead a single structured document that you review, commit, and reuse.

Claude Code shipped `/team-onboarding` in the week of 2026-04-06 to 2026-04-10 "for packaging your setup" ([Claude Code documentation index](https://code.claude.com/docs/llms.txt)). The command is one implementation of the artifact pattern. The durable shape is tool-agnostic: any agent that can read the repository and a curated set of usage traces can produce the same output.

| Dimension | Interactive Q&A | Generated artifact |
|---|---|---|
...
The Claude Code best-practices guide frames this kind of durable artifact — `/init` "analyzes your codebase to detect build systems, test frameworks, and code patterns, giving you a solid foundation to refine" and the resulting CLAUDE.md "compounds in value over time" when checked into git ([Claude Code best practices](https://code.claude.com/docs/en/best-practices)). The onboarding guide is the reader-facing sibling to CLAUDE.md: CLAUDE.md tells future agents how to behave; the ramp-up guide tells future humans how to read the code.

## Why synthesize instead of Q&A

An agent with codebase search and file-reading tools can assemble a repository-wide view in one synthesis pass, then compress it into a map people can read. [RepoAgent (arXiv 2402.16667)](https://arxiv.org/abs/2402.16667) formalizes this as a framework for LLM-generated repository-level documentation, motivated by the observation that hand-written docs drift from implementation within weeks. The artifact amortizes the comprehension cost: you review once, and it serves every later reader.
...
- Reviewers can audit the claims the artifact makes, catching errors early rather than letting them spread through individual onboarding sessions.
- Git history of the artifact itself becomes a signal — when the file changes significantly between regenerations, that is a prompt to tell the team about the architectural shift.
...
Regeneration-on-trigger bounds drift by the frequency of significant change rather than by an arbitrary calendar. Between triggers, the artifact is stable enough to cite.

## When this backfires

The artifact pattern has specific failure conditions. Prefer the interactive Q&A form, or skip onboarding tooling entirely, when any of these apply:
...
- Rapidly-changing [greenfield codebases](agent-driven-greenfield.md) — the artifact is stale before the next teammate arrives. Live Q&A works better because it reflects current code.
...
- Codebases dominated by tacit knowledge — the agent reads only what is in the repo. It cannot extract judgments that live in senior engineers' heads. The artifact will look complete while missing the conventions that actually matter. Pair with [encoding tacit knowledge](encoding-tacit-knowledge.md) rather than relying on synthesis alone.
- Over-reliance that deepens comprehension debt — if newcomers read only the artifact and never look at the source, [comprehension debt](../patterns/anti-patterns/comprehension-debt.md) builds up. Treat the guide as a map, not a substitute for the terrain.

## Review discipline

Treat the generated guide like any agent output: the artifact must pass review before it enters the repo. Specific checks:
...
The review is lighter than writing the guide from scratch, heavier than rubber-stamping. That is the trade the artifact pattern buys.
...
In review, the module owner compares the diff against the previous version. Two sections changed materially: the entry-points section now lists two services instead of one, and the glossary adds "capture sidecar" with a link to the new module. The reviewer spot-checks three claims against the code. The first entry point exists at the claimed path, and the second is correct. A claimed "retry policy convention" is accurate in two adapters but overgeneralized as universal, so the reviewer corrects the phrasing to "retry policy convention in processor adapters, see `adapters/stripe.ts` for the canonical example."

The corrected artifact then lands as `docs/onboarding/ramp-up.md`. The next teammate who joins reads it before opening their first file.
...
- The artifact pattern shifts onboarding from repeated individual Q&A to one-shot synthesis plus amortized review, producing a version-controlled map that survives across teammates.
- The durable artifact typically covers entry points, hot files, conventions, must-read history, and glossary — the reader-facing content an agent with repository access produces from the same scan that generates CLAUDE.md.
...
- [Agent-Powered Codebase Q&A and Onboarding](codebase-qa-onboarding.md) — the interactive complement to the artifact pattern
- [Team Onboarding for Agent Workflows](team-onboarding.md) — team-coordination onboarding, distinct from the artifact
...
- [Getting Started with Instruction Files](../instructions/getting-started-instruction-files.md) — CLAUDE.md and AGENTS.md are sibling artifacts to the ramp-up guide
