...
Forcing agent work into public channels makes the conversation a [searchable artifact](encoding-tacit-knowledge.md). The same transcript that solves one person's problem becomes the example the next person learns from without asking. Tobi Lütke calls the resulting environment a 'Lehrwerkstatt' — a teaching workshop. "The whole shop floor is the classroom. You learn by being near the work." ([Simon Willison, Learning on the Shop Floor, 11 May 2026](https://simonwillison.net/2026/May/11/learning-on-the-shop-floor/))

Two reinforcing loops produce the gain:
...
The deployment is not a pilot. In a single 30-day window 5,938 Shopify employees engaged with River across 4,450 public channels, and River authored about 1 in 8 merged pull requests in the main monorepo ([ZenML LLMOps Database](https://www.zenml.io/llmops-database/building-a-public-ai-agent-workspace-for-organizational-learning)). The merge rate improved from 36% to 77% over two months. Shopify attributes the gain not to model swaps but to organizational learning: people watch River work, find where it got stuck, and write the skills and instructions it should have had. A skill written by one team to teach River about the checkout data warehouse was reused by twelve other teams ([ZenML LLMOps Database](https://www.zenml.io/llmops-database/building-a-public-ai-agent-workspace-for-organizational-learning)).
...
Screen for these failure conditions before adopting the pattern:
...
- Small teams, under 10 engineers — the diffusion benefit comes from non-participating observers. Below ten engineers every conversation already has a known audience, and the ceremony adds cost without gain.
- Active incident response — incident channels are not the place to experiment publicly with an agent. The pattern's noise competes with signal at the worst possible moment.
- Public question channels that go dead under status pressure — without active norm-setting, even nominally public channels collapse. People prefer to ping experts privately ([Question Base — Slack Knowledge Sharing](https://www.questionbase.com/resources/blog/team/slack-knowledge-sharing-building-trust-scale)).
...
The minimum viable form for a smaller team: one named channel per agent-using practitioner (for example, `#alex-with-claude`), the agent configured to refuse DMs with a polite redirect, no curriculum and no required participation. The pattern's value is in the option to observe, not in mandated viewing.
