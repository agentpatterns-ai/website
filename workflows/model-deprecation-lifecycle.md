...
- Cross-provider fallback: if both Anthropic and a provider-routed Copilot path are configured, failing one provider's successor routes to the other. This is cheap when [provider abstraction](../token-engineering/cost-aware-agent-design.md) is already in place.
...
- Day 8–21: deploy canary at 10% traffic. Compare error rate, p95 latency, and token cost against the 90% slice still on `claude-sonnet-4-20250514`. Normalize cost on output tokens since the tokenizer is stable across Sonnet 4.x.
...
- Regression evals must cover drift-prone dimensions: response length, literal instruction following, tool-call frequency, subagent spawning, effort-level honoring.
