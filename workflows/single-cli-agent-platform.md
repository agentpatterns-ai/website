...
- CLI version coupling. Bundling init, eval, and deploy in one binary means a breaking change in any subcommand forces the whole project to re-pin, where a fragmented stack would let each phase update independently. The MCP-over-HTTP alternative stays preferable where centralized OAuth, RBAC, and audit logging matter ([Tyk: MCP vs CLI for AI Agents](https://tyk.io/learning-center/mcp-vs-cli-for-ai-agents-enterprise-comparison-guide/)).
