...
Pick natural-language semantic search when intent dominates the query rather than exact tokens:
...
- Duplicate detection before opening a new issue: a paraphrased description is exactly the shape an embedding index handles well — the [find-before-create check that keeps the backlog clean](backlog-triage-skill.md).
- Triage on an unfamiliar repo: you do not know the project's preferred labels, components, or wording conventions.
- Conversational scoping inside an existing chat: the agent is already in the chat surface, and switching out to the issues UI breaks the loop.

## When to skip it

Stay on keyword or Boolean search when literal precision, completeness, or determinism dominate:
...
Semantic issue search pays off most in duplicate detection inside an agent triage loop. The GitHub Copilot SDK Issue Triage Agent already runs this shape: on every newly opened issue it searches the existing corpus for similar reports and closes confirmed duplicates ([DeepWiki: Issue Triage Agent](https://deepwiki.com/github/copilot-sdk/9.2-authentication-and-billing)).
