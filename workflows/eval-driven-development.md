...
Learn it hands-on with [Define "Done" First](https://learn.agentpatterns.ai/workflows/define-done-first/) — a guided lesson with quizzes.
...
Before writing any agent feature code, do three things:
...
- Highly subjective outputs with shifting preferences: when success hinges on evolving aesthetic or stylistic judgment (for example, creative copy, UX tone) that changes faster than the eval set can be updated, the suite misleads more than it informs — tasks pass while real users dislike the output.
- Unstable upstream dependencies: if the tools, APIs, or data sources the agent depends on churn weekly, the eval set breaks faster than it yields signal — the same drift that destabilizes [tool-building evals](#applying-the-loop-to-tool-building). Defer formal evals until the environment stabilizes.
