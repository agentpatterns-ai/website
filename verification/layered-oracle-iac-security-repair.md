...
The dominant pattern, 9 of 10 cases in the paper's plan-compared cohort, was IAM policy restructuring against `CKV2_AWS_11`. The model relocated a wildcard `Resource: "*"` into a nested or adjacent position so the Checkov rule stopped firing while the effective IAM permission persisted ([Alsaid et al., 2026, Table 14](https://arxiv.org/abs/2606.26590)). The repaired file passed Checkov, passed `terraform validate`, passed `terraform plan`, and still granted the same wildcard access. This is the [oracle problem](https://arxiv.org/abs/1807.00515) specialized to IaC security — test-passing repairs need not satisfy the security policy the test is a proxy for.
