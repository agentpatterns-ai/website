...
- The per-trace judge is below the precision floor. AgentRewardBench measured 12 LLM judges on 1,302 web-agent trajectories, and none cleared human inter-annotator agreement, with errors clustering around grounding mismatch and misunderstood actions ([AgentRewardBench, 2025](https://arxiv.org/abs/2504.08942v2)). TRAIL found long-context LLMs score only 11% on trace-debugging tasks ([TRAIL, 2025](https://arxiv.org/abs/2505.08638v3)). Macro aggregation amplifies these errors, so clusters become recurring judge mistakes that look like system behavior.
