...
The first three are positive obligations — a witness proves the edge fires. Restricted-tool coverage is adversarial: a scenario that *should* induce the restricted call is generated, and the witness records whether the restriction held. That criterion elicited **23/248 restricted-call violations (9.3%)** — concrete misrouting failures prompt-level checks had missed.
