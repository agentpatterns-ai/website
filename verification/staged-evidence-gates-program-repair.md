...
3. **The retrieval scaffold's corpus is in-distribution for the bug.** Cross-cutting refactors, repairs that require non-repository knowledge, or sparse-context bugs degrade retrieval — the [Skill Retrieval Realism Gap](skill-retrieval-realism-gap.md) reports that realistic retrieval drops accuracy substantially below benchmark numbers.
...
- **Out-of-distribution retrieval**: cross-cutting refactors and bugs requiring non-repository knowledge degrade the retrieval leg; the scaffold grounds the agent in misleading context. The [Skill Retrieval Realism Gap](skill-retrieval-realism-gap.md) quantifies the production degradation.
...
- The retrieval scaffold reduces invalid candidates upstream; the compile gate rejects syntactically incoherent patches; the target-test gate confirms symptom relief before paying for the full suite.
