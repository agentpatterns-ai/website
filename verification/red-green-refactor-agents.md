...
Green — instruct the agent to write the minimum implementation to pass the suite. "Do not change the tests" stops the agent passing tests by weakening them, the [reward-hacking](anti-reward-hacking.md) shortcut models reach for. Exit: all tests pass.
...
- "Minimum code to pass" — prevents over-engineering in the green phase. Save complexity for the refactor phase, where you can evaluate it against known-good behavior.
