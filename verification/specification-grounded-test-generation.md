...
- [Spec-Driven Test Generation: Contract Coverage Is the Lever](spec-driven-test-generation-contract-coverage.md) — the same grounding when no human writes the rules: the agent extracts the contract itself, and detection tracks whether that contract named the violated condition
