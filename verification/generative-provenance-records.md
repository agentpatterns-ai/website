...
The three fields are the load-bearing primitives, and the third is the one that makes mechanical verification possible. The relation taxonomy from [TRACER](https://arxiv.org/abs/2605.09934):

- **Quotation** — direct reuse of explicit text in the tool observation, minimal transformation
- **Compression** — faithful condensation of a longer observation, no new reasoning introduced
...
1. **Schema validation** — the record is well-formed JSON with required fields. Catches malformed output, missing indices, dropped provenance items.
2. **Tool-turn alignment** — the cited `tool_turn_id` is present in the trajectory. Catches fabricated turn IDs and hallucinated tool calls.
3. **Source authenticity** — the `evidence_span` actually appears in that turn's output (substring or localized region match). Catches fabricated quotes and misattributed observations.
4. **Relation rationality** — the labelled relation is consistent with the claim-evidence pair. For *Quotation*, a substring match suffices; *Compression* and *Inference* require a tighter check, usually a small judge model with the evidence and claim in front of it.

Checks 1-3 are deterministic string operations on JSON. They cannot be fooled in the way an LLM judge can. The relation taxonomy narrows check 4 — verifying "this is a Quotation" is far cheaper than asking "is this faithful?" in the abstract. This is the same mechanism documented in [Citation-Grounded Code Comprehension](https://arxiv.org/html/2512.12117v1), where upfront constraint enforcement produced "zero false negatives in 1,080 verified responses, confirming architectural prevention superior to post-hoc detection."

## Where to Apply It

This pattern earns its overhead when **tool observations are unstructured natural language or multimodal blobs the agent must summarise**:
...
- Coding agents calling `pytest`, `git diff`, the type checker — the trajectory log already is the audit trail, and a sentence like "the failing test was `test_foo`" is verified by re-running the test, not by a relation-labelled record
