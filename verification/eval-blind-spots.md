...
Run-to-run variance is itself a measurement-methodology problem that sits upstream of the four structural gaps: OpenAI documents an approach for separating real signal from run-to-run noise in coding-agent evaluations, so a single scored pass is not mistaken for a genuine capability difference ([OpenAI: Separating signal from noise in coding evaluations](https://openai.com/index/separating-signal-from-noise-coding-evaluations/)). Microsoft frames the same problem as hidden variables — naming model temperature, file ordering, and shared API state as non-determinism sources that silently move eval scores unless the harness controls for them ([Microsoft: The hidden variables in your agent eval](https://developer.microsoft.com/blog/the-hidden-variables-in-your-agent-eval/)). Context settings belong on that list too: retaining private reasoning across tool calls and compacting instead of truncating moved one model from 13.3% to 38.3% on a public benchmark without changing the model ([OpenAI, 2026](https://openai.com/index/how-two-settings-tripled-our-arc-agi-3-scores)), so a leaderboard gap can be [a harness difference rather than a capability difference](../context-engineering/reasoning-retention-and-compaction.md).
