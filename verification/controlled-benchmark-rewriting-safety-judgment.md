...
- Implicit risks — explicit cues scrubbed, so the underlying mechanism (privilege escalation, data exfiltration, irreversible side-effect) stays the same but no longer trips surface tokens.
...
- No curated trajectory bank — ARISE needs 2,000+ ReAct-style exemplars. Teams without one fall back to standard RAG, and rewriting alone still yields the diagnostic signal.
...
- Latency-sensitive inline guards — top-3 retrieval adds context tokens to every judgment call, which rules it out for high-throughput filters.
- Surface over substance — a guardrail that learns to spot "appeals to authority" still pattern-matches a register. Pair it with [trajectory-aware auditing](eval-blind-spots.md) on real traces.
