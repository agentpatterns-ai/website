...
The [Symphony spec](https://github.com/openai/symphony/blob/main/SPEC.md) defines a long-running service. It polls an issue tracker, claims eligible issues, prepares a workspace for each issue, runs an agent session in each, and reconciles state. The spec owns five concerns:
...
| Workspace lifecycle | Sanitized per-issue paths under a configured root, with `after_create` / `before_run` / `after_run` / `before_remove` hooks |
...
A repo-owned `WORKFLOW.md` configures Symphony. It carries YAML front matter that declares tracker, polling, workspace, hooks, agent concurrency, and codex settings. It also carries a Markdown prompt body with `{{ issue.* }}` and `{{ attempt }}` variables ([SPEC.md](https://github.com/openai/symphony/blob/main/SPEC.md)).
...
- Workspace directory keys are sanitized to `[A-Za-z0-9._-]`; other characters become underscores.

Approval, sandbox, and user-input policies are deliberately implementation-defined. Each implementation must document the posture it chooses.
...
- Tracker adapter v1 supports Linear only. The tracker interface is generalizable, but no other adapter ships.
- Agent runtime v1 supports Codex App Server only. The launch contract hardcodes `codex app-server` and assumes the Codex session and turn protocol.
- The reference implementation is Elixir. OpenAI invites reimplementation but does not commit to maintaining Symphony as a standalone product.
- The status is early. OpenAI describes it as a "low-key engineering preview for testing in trusted environments" — not production-ready.
...
Each Linear issue in `Todo` or `In Progress` is claimed, given a sanitized workspace, and handed to a Codex session with the rendered prompt. The claim releases when the issue reaches a terminal state.
