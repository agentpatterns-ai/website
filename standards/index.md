...
- [Agent Plugins: Portable Packaging With Client-Defined Trust](agent-plugins-standard.md) — A vendor-neutral directory format for shipping Agent Skills and MCP servers across clients, which leaves installation, provenance, and permissions to each client
...
- [Stateless MCP: One Request per Tool Call](stateless-mcp.md) — The 2026-07-28 MCP revision drops the initialize handshake and Mcp-Session-Id, so a tool call is one self-describing HTTP request any server instance can answer
...
- [WebMCP: Browser-Hosted Tool Contracts for In-Page AI Agents](webmcp.md) — A W3C Community Group draft that lets a web page register JavaScript tools through `navigator.modelContext` so an in-tab browser agent calls them by name instead of actuating the DOM
