...
Comparing `security-guidance` against an alternative that adds 800 tokens always-on and ships three more skills plus two more hooks shows both the 620-token cost gap and four extra components — neither observable post-install without paying both costs and running `claude plugin details` ([Plugins reference](https://code.claude.com/docs/en/plugins-reference)).
