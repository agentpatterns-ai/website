...
[A2UI](https://a2ui.org/) is an Apache 2.0 spec from Google for describing UI as structured data. An agent emits JSON referencing components from a catalog the host advertises; the host renders them with its native widgets. [Google released v0.8 in December 2025](https://developers.googleblog.com/introducing-a2ui-an-open-project-for-agent-driven-interfaces/) and [shipped v0.9 on April 17, 2026](https://developers.googleblog.com/a2ui-v0-9-generative-ui/) with renderers for Flutter, Lit, Angular, and React plus a Python Agent SDK.

It sits alongside the other agent-facing open standards: [llms.txt](llms-txt.md), [AGENTS.md](agents-md.md), [Agent Skills](agent-skills-standard.md), [MCP](mcp-protocol.md), and [A2A](a2a-protocol.md). A2UI covers the agent-to-UI handoff.
...
Neither has won. [An upcoming bridge lets MCP servers return A2UI blueprints alongside HTML resources](https://www.copilotkit.ai/blog/the-state-of-agentic-ui-comparing-ag-ui-mcp-ui-and-a2ui-protocols), suggesting coexistence rather than head-to-head competition.
