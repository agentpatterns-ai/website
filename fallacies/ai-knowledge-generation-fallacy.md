...
Teams treat AI-generated test suites as a primary quality signal. Tests generated by the same model that wrote the code reflect the model's expectations, not ground truth. They come from one training distribution, so they give no independent signal about correctness.

Teams request architecture decisions from brief prompts. AI [surfaces documented patterns](../patterns/anti-patterns/pattern-replication-risk.md) from training. For systems with novel constraints, those patterns may be well-known but a poor fit. The model cannot know what it has never seen.

Teams bootstrap fine-tuning and eval sets from AI outputs. [Model collapse](https://pubmed.ncbi.nlm.nih.gov/39048682/) is the structural result: each generation trained on AI-generated content loses information, as tail events disappear and outputs converge on generic responses. Shumailov et al. (Nature, 2024) showed this across LLMs, VAEs, and Gaussian mixture models. As web-scraped data turns synthetic, the effect compounds across the wider web.

Teams request domain documentation for domains outside AI's training data. AI cannot produce domain knowledge it never saw. Asking it to document proprietary systems or new regulatory frameworks produces plausible-sounding confabulation.
...
What matters is whether the problem needs new information, or competent retrieval of existing practice.
