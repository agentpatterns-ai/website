...
**Architecture decisions from brief prompts** — AI [surfaces documented patterns](../anti-patterns/pattern-replication-risk.md) from training. For systems with novel constraints, the retrieved patterns may be well-known but ill-fitted. The model cannot know what it has never seen.
...
- **Recombination as the goal**: Many real tasks — boilerplate, [test scaffolding](../anti-patterns/happy-path-bias.md), documentation, code translation — need competent recombination, not invention. The warning applies only when you need genuinely novel output.
