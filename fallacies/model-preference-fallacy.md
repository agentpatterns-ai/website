...
- **The "preference" comes from an evaluation method robust to surface variance.** "Flaw or Artifact?" argues that much of the apparent prompt sensitivity in the literature stems from heuristic evaluation methods — log-likelihood scoring and rigid answer matching — that fail to recognise semantically equivalent responses; LLM-as-a-Judge evaluations show reduced variance and more consistent rankings. [Source: [Flaw or Artifact? Rethinking Prompt Sensitivity in Evaluating LLMs](https://arxiv.org/abs/2509.01790)] A comparison that uses an LLM judge tolerant to phrasing differences is on firmer ground than a strict-match heatmap.
