...
- [Agent Development Lifecycle for Agent Products](agent-development-lifecycle.md) — A four-phase meta-lifecycle (build, test, deploy, monitor) where verdict-labelled production traces feed the next evaluation and build cycle
...
- [Abstention-Aware Memory Retrieval](abstention-aware-memory-retrieval.md) — Treat memory injection as a risk-sensitive control decision — abstain or fall back to no-memory when retrieval evidence is weak rather than always using top-k
...
- [Goal-Driven Autonomous Loop with Budget Cap](goal-driven-autonomous-loop.md) — An objective-bound agent loop that injects a continuation prompt and budget telemetry at each turn end, stopping when the agent declares the goal complete or a token budget is exhausted
...
- [Gateway Model Routing](gateway-model-routing.md) — Treat an Anthropic-compatible gateway as both inference target and model catalogue, so a single config knob controls what the harness can call and what it shows in the picker
