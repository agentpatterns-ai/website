...
- [Agent Project State Purge: Clean-Slate Session Reset](agent-project-state-purge.md) — A clean-slate primitive that tears down per-project session state — transcripts, auto-memory, indexed sessions — when contamination is the real diagnosis, not an instruction-file bug
...
- [Typed Memory from VCS History](typed-memory-from-vcs-history.md) — Distil git commits, PR discussions, and issues into Facts/Skills/Patterns with a budget-constrained TF-IDF retriever — works only under tight retrieval budgets and high commit hygiene; otherwise converges with BM25
...
- [Goal Contract: Separating the Doer from the Done-Checker](goal-contract-completion-evaluator.md) — A user-declared completion condition watched by a separate evaluator model after every turn — completion authority lives outside the agent doing the work
...
- [Adaptive Generate-Rank-Verify](adaptive-generate-rank-verify.md) — Generate cheaply, rank with a cheap signal, spend the expensive verifier only on the top of the ranking under a doubling schedule — when verification dominates per-sample inference cost
