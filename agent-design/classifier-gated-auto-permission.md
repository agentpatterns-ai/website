...
If any condition fails, simpler postures dominate: a static allowlist plus sandbox for low-volume sessions, or [Bootstrap Evidence-Based Allowlist](../agent-readiness/bootstrap-evidence-based-allowlist.md) for steady-state workloads where the action distribution is enumerable.
...
The empirical effect is the rubber-stamp problem inverted. Codex sessions stop for human approval roughly **200× less often** in auto-review mode than in manual mode while still catching the actions that should stop — in a 10,000-action snapshot, 9,280 ran inside the sandbox without review; 720 escalated to auto-review; the reviewer approved 713, denied 7, and the doing agent recovered safely on 4 of the 7 denials ([Auto-review of agent actions](https://alignment.openai.com/auto-review/)). The two-stage internal pipeline used by Claude Code's auto mode — a single-token filter tuned for recall followed by chain-of-thought reasoning only on flags — keeps the cost shape compatible with high-volume traffic ([Anthropic engineering](https://www.anthropic.com/engineering/claude-code-auto-mode)).

## Four Design Choices That Decide the Trade

Whether a given implementation earns its complexity depends on four choices:

1. **Reviewer isolation from the doing agent.** Same training distribution means the same jailbreak defeats both — measured at 64.5% blind-spot rate when models review their own outputs ([Panickssery et al., 2025](https://arxiv.org/abs/2507.02778)). Use a different model class, or at minimum reasoning-blind the reviewer so the doing agent cannot argue it down.
...
3. **Coverage boundary.** Whatever bypasses the classifier (Tier 1 and Tier 2 above) is the system's residual blast radius. When ops state lives inside the project root — terraform state, sealed-secrets manifests, k8s YAML — Tier 2 becomes an exfiltration and destruction surface the classifier never sees. Ji et al. measured 36.8% of state-changing actions bypassing classification this way ([Measuring the Permission Gate](https://arxiv.org/abs/2604.04978)).
4. **Audit trail.** Every auto-approved action must be reconstructable post-hoc — the reviewer's verdict, the rule it matched, and the resolved arguments. Without per-action telemetry, the classifier's decisions are unauditable and the layer provides false confidence. OpenAI's Codex posture wires OTel exports for prompt, approval decision, execution result, and network proxy decisions as the fourth load-bearing leg ([Running Codex safely at OpenAI](https://openai.com/index/running-codex-safely/)). Pair with [Audit Confirmation Gate Logs](../agent-readiness/audit-confirmation-gate-logs.md) for the gate-decision side.

## When This Backfires

- **Workload outside the calibration distribution.** Anthropic's 17% FNR is on a hand-curated overeager set of 52; Ji et al.'s 81% FNR is on 128 deliberately ambiguous DevOps prompts and 92.9% on artifact-cleanup tasks ([Measuring the Permission Gate](https://arxiv.org/abs/2604.04978)). The classifier is a probabilistic floor calibrated against typical traffic, not a determinative one against an adversary.
- **In-project ops state.** Tier 2 becomes a coverage gap when the project root contains production-affecting files. Either move that state outside the project root, or pin the relevant paths with deterministic [permission-framework-over-model](../security/permission-framework-over-model.md) rules.
- **Same model class for agent and classifier.** Shared jailbreak surface. Reasoning-blinding mitigates argument-based bypass but not training-distribution-based bypass.
...
- **Transitive supply-chain attacks.** The classifier evaluates actions, not outcomes. `pip install -r requirements.txt` is exactly what the user asked for; the classifier cannot reason about what declared dependencies pull in.
...
- [Bootstrap Evidence-Based Allowlist](../agent-readiness/bootstrap-evidence-based-allowlist.md) — the static-allowlist alternative for steady-state, enumerable action distributions
- [Audit Confirmation Gate Logs](../agent-readiness/audit-confirmation-gate-logs.md) — the telemetry layer that makes classifier decisions reconstructable post-hoc
