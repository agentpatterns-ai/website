...
- **Policy-Gated Tool Invocation without risk scoring under-enforces.** `PreToolUse` hooks plus a static deny list collapse OAS's five stages into two. A deny list cannot distinguish `rm -rf /tmp/cache` from `rm -rf /` unless you hand-author the matcher, and industry data indicates that inline runtime risk scoring is the stage most production setups omit — [Gravitee's State of AI Agent Security 2026 report](https://www.gravitee.io/blog/state-of-ai-agent-security-2026-report-when-adoption-outpaces-control) finds only 23% of organizations enforce AI security inline at the point of action.
