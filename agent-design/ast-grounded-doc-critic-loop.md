...
**Dependency-aware retrieval** (the RAG layer) populates the context window with exactly the code that constrains the doc. Whole-module loading pollutes context; loading nothing forces invention. The AST answers precisely: load the transitively-referenced nodes.
...
**Drift-loop churn** — stylistic rejection criteria make the generator oscillate between equivalent phrasings without converging. Cap iteration depth and escalate non-convergence (see [Convergence Detection](convergence-detection.md)).

**AST coverage gaps masquerading as agreement** — symbols generated by macros, decorators, or runtime metaclasses appear partial or absent in the AST. The critic sees no diff to disagree with and the loop terminates with a doc that misses actual runtime behaviour. Track which doc symbols map to AST nodes ([Code-Native Memory Substrates](code-native-memory-substrates.md) mapping); flag unresolved ones.
