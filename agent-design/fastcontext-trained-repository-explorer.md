...
Context isolation along the exploration-versus-solving seam, plus citation-precision training. The solver pays for every token in its window; exploration is verbose. Routing exploration into a separate sub-agent keeps verbose intermediate state out of the solver — only the *citations* (tens of tokens each) cross back. This is the same isolate-then-distil mechanism Anthropic names for sub-agent fan-out generally ([Anthropic: effective context engineering](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)), specialised to the repo-exploration axis with a trained model.
...
| [Sub-Agents Fan-Out](../multi-agent/sub-agents-fan-out.md) | Per parallel dispatch | The general fan-out primitive; FastContext is a specialisation to repo exploration |
...
FastContext's distinguishing properties: (a) the explorer is **trained** with task-grounded rewards on citation precision, not just prompted, and (b) the output contract is **file-path + line-range citations only** — never raw snippets — which is what makes downstream context isolation tight.
