...
The self-healing loop automates the first three steps of the incident cycle and keeps human judgment for the merge gate alone. The classic cycle — detect, ticket, triage, fix, review — needs a person at every hand-off.
...
LangChain applied this to their GTM Agent: after every deploy, the system compares eval scores before and after the deploy, triages causality, and opens a fix PR ([LangChain: How My Agents Self-Heal in Production](https://blog.langchain.com/production-agents-self-heal/), April 2026).
...
The current deploy does not cause every regression. A triage agent — the attribution step in LangChain's GTM Agent loop — compares the failure against pre-deploy baselines to find the cause:
...
- Low deploy frequency: teams shipping once a week or less rarely need automated causality triage — the deploy-to-failure gap is wide enough for manual triage to stay fast.
- No stable eval suite: the loop needs a fixed set of deterministic graders to compare scores before and after a deploy — the same stable suite [harness hill-climbing](harness-hill-climbing.md) tunes against. If the eval suite itself changes between deploys, the causal diff becomes unreliable and triage produces false positives.
...
- High flake rate in the test suite: intermittent failures that no deploy caused trigger the triage agent repeatedly, exhaust the circuit breaker budget, and escalate noise to human tickets — the opposite of what the pattern promises.
