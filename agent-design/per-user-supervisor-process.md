...
This is [session, harness, and sandbox separation](session-harness-sandbox-separation.md) projected onto local desktop tooling. Anthropic's case for splitting along layers whose churn rates differ by orders of magnitude argues that "a monolithic agent process couples model inference, session state, and execution environment. When any one churns — models shift, execution targets multiply, or a crash forces recovery — the others pay the cost" ([Anthropic: managed agents](https://www.anthropic.com/engineering/managed-agents)).
...
- **Power-cycle vs sleep.** "Shutting down or restarting your machine stops running background sessions, so they show as failed when you next open agent view" ([agent view docs](https://code.claude.com/docs/en/agent-view#sessions-show-as-failed-after-shutdown)). Sleep is recovered; shutdown is not.
