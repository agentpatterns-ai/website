...
The full SKILL.md is five sentences. What makes it work is firing at an explicit transfer point with a no-duplication rule that prevents the most common failure: copying the whole transcript.
...
Fire-and-forget substitutes for a sub-agent primitive when the harness lacks one. DIY-sub-agent is a manual round-trip when the harness doesn't surface a structured fork.
...
Recap fires when the harness detects a boundary. The pipeline protocol fires on a fixed stage edge. The handoff skill fires when *you* decide to transfer — the model dispatches it based on its description, not a hook or a graph edge.

## What the Handoff Document Carries

The reference SKILL.md is prescriptive about what *not* to write — duplicating PRDs, ADRs, or commit diffs inflates the doc without adding decision-density. The doc captures the parts that exist *only* in the conversation:
...
- [Audit Handoff Protocols](../agent-readiness/audit-handoff-protocols.md) — How to validate the receiver's expected schema matches what the skill produces
