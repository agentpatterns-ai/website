...
**Before — green check read as the go-ahead:** a skill scans clean, so it is installed. Its malicious step was rendered as a "workflow diagram" image the static scanner never parsed, and the multimodal agent reads and executes it at runtime ([Jia et al., 2026](https://arxiv.org/abs/2606.18198)). The pass certified "no pattern matched," which was never the same as "safe."
...
- [Benign Skill Wording That Steers Package Hallucination (Neutral Prompting Attack)](../../security/neutral-prompting-hallucination-steering.md) — the measured version of that case: static scanners flag nothing, and ten of ten human reviewers rated the skill as unsuspicious.
- [Treating a Clean Static Scan as Security Evidence](static-clean-as-security-evidence.md) — the same green-check error applied to generated code rather than to skills, with runtime exploit tests putting a number on what the scan missed.
