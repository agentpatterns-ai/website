...
- [Explanation-Bound Tool Execution for Agent Gateways](../../security/explanation-bound-tool-execution.md) — the same pre-execution placement applied to the agent's stated reason, checking typed claims against server-held facts rather than the call against state
