...
- Ultra-short tool calls. Sub-reasoning-step tool latency leaves no window to fill, so the technique falls back to vanilla while still paying scheduler overhead. 25 to 27% of GAIA tool calls hit this regime ([arXiv:2605.22154](https://arxiv.org/html/2605.22154v1)).
- Metered, cost-bound workloads. Latency parity is real, but discarded draft branches are pure token spend. The "no latency overhead" headline does not extend to dollar cost, because speculative drafting "consumes additional LLM tokens during the idle window" ([arXiv:2605.22154](https://arxiv.org/html/2605.22154v1)).
