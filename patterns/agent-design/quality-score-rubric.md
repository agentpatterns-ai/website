...
The same A/B/C/D framing works for the agent, not only the human reviewer: LangChain's rubrics let an agent evaluate and correct its own work against an explicit rubric ([LangChain, introducing rubrics](https://blog.langchain.com/introducing-rubrics)).
...
End-of-session updates land while state is still top-of-mind. A `Stop`-event hook can emit a grade hint from `lint --check` exit codes, test pass rate, or defect counts; a human or supervisor agent ratifies it into the rubric. The [pre-completion checklist](../../verification/pre-completion-checklists.md) pattern is the same wiring point. The hook should propose, not write — automated grade rewrites turn the rubric into a moving average of recent build state, which loses the "uncomfortable truth" the rubric is meant to surface.
