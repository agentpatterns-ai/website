...
- Side-channel substitute. Routing observations to a fenced low-priority queue (a JSONL file, a draft-issue label) is the steelman counter-position when the team distrusts agent-filed work in the main tracker. It preserves the externalization but adds a human promotion step — useful as a transitional posture, not a permanent end state.
