...
- Single-vendor team that only ever speaks one envelope. You get three code paths where one would do, and the legacy `customoai` adapter was a strictly smaller surface for these teams. The provider abstraction only pays off when the BYOK pool actually spans shapes.
