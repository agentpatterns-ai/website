...
- Beats MIPROv2 by over 10% on average, and by 12% on AIME-2025.
...
A RAG pipeline where the failure modes are "wrong retrieval" versus "correct retrieval but wrong answer synthesis." A scalar exact-match metric cannot distinguish them, so GEPA starves.
...
- [Cost-Aware Agent Design](../../token-engineering/cost-aware-agent-design.md) — choosing optimizer complexity against runtime volume
