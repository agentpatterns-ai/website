...
- Snapshot staleness drift — long-lived snapshots mask dependency churn the same way stale prebuilt images do. The agent then runs against tooling that diverges from what developers see locally. Scheduled rebuilds with a promotion gate address this directly; see [Continuously Built Agent Environments](continuously-built-agent-environments.md).
