...
Default SDK behavior reads `CLAUDE.md` memory and settings from the filesystem, leaking across tenants in a shared container. The isolation contract requires all four simultaneously ([Anthropic: Hosting §Multi-tenant isolation](https://code.claude.com/docs/en/agent-sdk/hosting)):
