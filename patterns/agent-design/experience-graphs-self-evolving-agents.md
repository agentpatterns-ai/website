...
The pattern is qualified. Independent results confirm gains, but only under specific conditions. Verify each before you adopt it:
...
2. Selective retrieval, not always-on. Across 7 backbones, 3 environments, and 8 benchmarks, ExpWeaver found that always-on injection of accumulated experience hurts. Gains come from retrieval triggered by decision uncertainty ([arxiv 2605.07164](https://arxiv.org/html/2605.07164v1)).
...
- G-Memory reports up to 20.89% improvement in embodied-action success rate and 10.12% in knowledge-QA accuracy across five benchmarks and three MAS frameworks ([arxiv 2506.07398](https://arxiv.org/abs/2506.07398v2)).
...
- Small-N regime. FSM design and the retrieval-breadth hyperparameter `k` require per-task tuning. Below some volume threshold the graph is pure overhead versus a flat episodic store.
