...
The slider lets an operator apply that same asymmetry reactively rather than architecturally. Before an interactive form existed, an operator who spotted a hard turn mid-session had to quit and relaunch with `--effort`, losing session context. Runtime adjustment removes that friction.
