...
1. Reviewer isolation from the doing agent. The same training distribution means the same jailbreak defeats both — measured at a 64.5% blind-spot rate when models review their own outputs ([Panickssery et al., 2025](https://arxiv.org/abs/2507.02778v3)). Use a different model class, or at minimum reasoning-blind the reviewer so the doing agent cannot argue it down.
