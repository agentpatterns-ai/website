...
- Silent failures dominate the surface. Structurally valid but wrong data needs cross-referencing, and recovery context measurably hurt here. The authors offer one unverified explanation, that the added context raises the model's propensity to retry and a refetched value is corrupted again, and say they "did not isolate this mechanism experimentally" and cannot rule out an alternative ([2608.11977v1](https://arxiv.org/abs/2608.11977v1)). [Outcome Monitors](outcome-monitors-recovery-affordances.md) attack this class from the other side, by detecting the violated property first and only then naming the substitute tools.
