...
1. Hub isolation — files with high in-degree or out-degree leave community detection and get their own partition. Without this step, one structural hub pollutes every community.
2. Community detection via Infomap — partition the remaining directed graph using the [Infomap algorithm](https://www.mapequation.org), which models the graph as a random walk and picks the partition that minimizes the two-level description length. Heavy edges stay inside partitions; cuts cross sparse edges.
3. Leaf lifting — independent leaf vertices move into singleton partitions when the community step missed latent parallelism.
...
The Infomap step does the heavy lifting. It treats the directed dependency graph as a random walk and selects the partition that minimizes the expected bits to describe that walk across boundaries — formally, the [map equation](https://www.mapequation.org). Minimizing bits-across-boundaries is the same as minimizing the "information leak" between partitions, which is the cross-agent context-transfer cost being optimized against ([arXiv:2606.00953](https://arxiv.org/abs/2606.00953)).
