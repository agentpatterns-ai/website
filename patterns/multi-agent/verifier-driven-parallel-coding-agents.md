...
- Materialized project overview — a generated read-only view of repository state that any agent can consult without conflicting with concurrent edits.
...
Per-agent instruction lapses are rare, but in a parallel system they compound. The probability that some agent violates some rule on some step approaches 1 as agents times steps grows, so prose coordination degrades quadratically with scale; verifier scripts short-circuit that scaling because they cost a process exec, not a context-window check ([Philippov et al., 2026](https://arxiv.org/abs/2606.27416v1)). The mechanism is the same one [deterministic guardrails](../../verification/deterministic-guardrails.md) use inside a single agent — telling an agent "don't break the build" is a prompt, running the build is a guardrail — extended outward to the contract between agents. Reported overhead is roughly 1% of wall-clock across three research campaigns; the verifier checks run fast against repository state and short-circuit before expensive agent work commits, so caught violations amortize the cost ([Philippov et al., 2026](https://arxiv.org/abs/2606.27416v1)). [Pre-completion checklists](../../verification/pre-completion-checklists.md) supply the same evidence at the per-agent layer; verifier-driven parallel agents apply the same logic at the inter-agent layer.
...
In the cited deployment, the framework coordinated up to twelve parallel agents across 273 tracked tasks in 129 feature sets for roughly $450 in API costs, with verifier scripts adding about 1% wall-clock overhead ([Philippov et al., 2026](https://arxiv.org/abs/2606.27416v1)). Applied to the BEA 2026 vocabulary-difficulty shared task, the system finished first on the closed track and second on the open track across three languages, reducing the official baseline RMSE by 29.9% (closed) and 35.9% (open) ([Philippov et al., 2026](https://arxiv.org/abs/2606.27416v1)).
...
- Four invariants carry the contract: task isolation, immutability of completed work, corrections overlay, materialized project overview.
...
- [Verify-Gated Completion as Admission Control](verify-gated-completion-admission-control.md) — the per-completion external-verifier sibling; this page is the rule-set generalization across all agents
...
- [Multi-Agent Topology Taxonomy: Centralized, Decentralized, and Hybrid](multi-agent-topology-taxonomy.md) — choosing the right coordination structure; verifier-driven contracts are an architectural choice within decentralized topologies
