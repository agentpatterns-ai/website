...
|-----|-----|-----|
| Fan-out / Fan-in (Scatter-Gather) | Sub-agent parallelism with result synthesis | [Fan-Out Synthesis](fan-out-synthesis.md) |
| Orchestrator / Worker | Lead agent decomposes task, delegates to specialists | [Orchestrator-Worker](orchestrator-worker.md) |
...
| Event-Driven Choreography | File-based coordination, hooks, pub-sub triggers | [File-Based Agent Coordination](file-based-agent-coordination.md) |
...
| MapReduce | Sub-agent fan-out returning condensed summaries | [Sub-Agents Fan-Out](sub-agents-fan-out.md) |
...
| Consensus Protocols | Generator-critic loops, voting, quorum review | [Evaluator-Optimizer](../agent-design/evaluator-optimizer.md), [Committee Review](../../code-review/committee-review-pattern.md), [Voting / Ensemble Pattern](voting-ensemble-pattern.md) |
...
3. Fan-out / fan-in — the orchestrator spawns one [sub-agent per module](fan-out-synthesis.md), each working in its own worktree. Results merge only after all sub-agents report success.
