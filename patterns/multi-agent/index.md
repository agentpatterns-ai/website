...
- [The Subagent Inheritance Contract: What Crosses Down](subagent-inheritance-contract.md) — Codex inherits the parent session's settings by default and Claude Code drops the conversation while injecting the rule hierarchy, so the unasked payload decides what a brief must carry
- [Role-Declared Context Mode: Where the Inherit Decision Lives](role-declared-context-mode.md) — deepagents fixes fork-or-isolated on the subagent definition and Claude Code requests it per spawn; declaring it keeps a verifier independent of the supervisor that dispatches it
...
- [Path-Scoped Write Contracts for Shared Agent State](path-scoped-write-contracts.md) — Name which role may mutate which state path and route every write through one validator; a schema-valid blackboard still admitted 52% of unauthorized writes
...
- [Rerunnable Claim Graph as Shared Agent Memory](rerunnable-claim-graph.md) — Publish each unattended agent's result as a commit a peer can check out and rerun, so verification costs a checkout; the record still does not stop duplicate work
