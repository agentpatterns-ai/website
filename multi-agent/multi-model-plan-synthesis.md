...
Frontier models have distinct training distributions, so they emphasize different parts of an architecture. Commit to one model's plan and you import its biases.

Running plans in parallel across 3 to 4 models is cheap: planning tokens are a small fraction of implementation tokens. The synthesis step turns model disagreement into a signal. Where models agree, the decision is high-confidence. Where they disagree, the tradeoff is worth examining.
...
Ask for git-diff style change annotations rather than a free-form rewrite. This forces precision and stops the synthesizer producing a vague summary that loses the specifics.

### 3. Tradeoff review

Inspect the disagreement map the synthesizer produces. Disagreements between models are architectural tradeoff candidates worth deliberate human review before implementation begins. Agreement signals decisions you can make with confidence.
...
The strongest case against this pattern: for most tasks, a single capable planner plus fast iteration exposes architectural errors earlier and more cheaply than any amount of pre-implementation synthesis. The pattern underperforms under specific conditions:

- Synthesizer bias dominates. The synthesis model anchors on its own perspective and silently downweights the other plans. The hybrid then mostly reflects a single training distribution while inheriting coordination overhead. This failure mode is hard to detect from the synthesis output alone.
