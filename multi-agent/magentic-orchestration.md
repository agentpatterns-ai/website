...
4. Specialists exposed to write-access tools run inside a sandbox with a pause-before-irreversible-action gate ([Microsoft Research — Magentic-One article](https://www.microsoft.com/en-us/research/articles/magentic-one-a-generalist-multi-agent-system-for-solving-complex-tasks/)).
...
When the stall counter exceeds the threshold (≤2 in the [Magentic-One paper](https://arxiv.org/html/2411.04468v1)), the inner loop breaks; the manager reflects, updates the ledger, and revises the plan before re-entering the inner loop.
...
- **Easy tasks** — the [Magentic-One paper](https://arxiv.org/html/2411.04468v1) authors note their system "appears to compete better on hard tasks vs. easy tasks," attributing this to fixed overhead that only amortises across long problems.
...
- **Sandbox by default.** Microsoft's reference implementation strongly advises running specialists "in isolated environments, such as Docker containers" ([microsoft/autogen-magentic-one](https://github.com/microsoft/autogen/tree/staging/python/packages/autogen-magentic-one)).
- **Pause before irreversible actions.** Microsoft Research explicitly recommends a human gate before file deletion, external API writes, or any action with no rollback ([Microsoft Research](https://www.microsoft.com/en-us/research/articles/magentic-one-a-generalist-multi-agent-system-for-solving-complex-tasks/)).
- **Fixed specialist roster.** The manager cannot dynamically create new agents; unused specialists distract it, and missing expertise has no fallback ([arxiv:2411.04468](https://arxiv.org/html/2411.04468v1) §Limitations). Curate the roster for the task type before deployment.
...
- Reported benchmarks: GAIA 38.0% (vs human 92.0%), AssistantBench 27.7% accuracy, WebArena 32.8% ([arxiv:2411.04468](https://arxiv.org/html/2411.04468v1)) — competitive but not human-level, and worse on easy tasks than hard ones.
