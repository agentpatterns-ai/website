...
An agent without a network tool is not automatically a closed-egress agent. If it can author messages on a surface whose renderer auto-fetches external resources, the renderer performs egress on the user's behalf. The lethal trifecta closes through *composition*, not a single tool grant.
...
3. The injection instructs the agent to post a Teams message with HTML `<img>` tags whose `src` attributes are attacker URLs carrying OneDrive pre-authenticated download links in the query string.
4. Opening the message causes the renderer to fetch the images, leaking the download links to the attacker's server.
...
The agent-side and renderer-side controls are orthogonal. The agent-side control is brittle — pattern matching misses redirect chains, data URLs, CSS background images, and `srcset` permutations. The renderer-side control matches the established email-client default and has a smaller policy surface, so teams that own both surfaces should apply both. Microsoft's guidance lists deterministic blocking of HTML image injection as a defence layer ([Microsoft Learn](https://learn.microsoft.com/en-us/security/zero-trust/sfi/defend-indirect-prompt-injection)) — but the Cowork incident shows it must be applied per surface: agent-authored Teams messages are a distinct surface from email arrivals processed by Copilot.

## When This Backfires

The pattern's defence work is wasted when the composition is already closed by another leg:

- **Trusted-by-design recipients**: If the only consumer is an operator on a plain-text-only inbox, no renderer auto-fetches resources and the exfil leg does not exist.
- **No private-data context**: An agent with untrusted input and an authoring tool but no access to sensitive corpora has nothing worth exfiltrating — the trifecta is already broken at leg 1.
- **Markdown-aware LLM consumers**: When the downstream consumer is another LLM that ingests markdown without auto-fetching resources, the renderer leg is closed by the consumer's nature.
- **Renderer outside your authority**: When a different vendor owns the renderer and you cannot force it to gate fetches, write-time URL stripping degrades into brittle pattern matching. Treat it as a tactical patch and assume residual risk.

Scheduled tasks compound the surface — a "weekly review" recurring task that loads a poisoned skill exfiltrates on every run without oversight ([PromptArmor disclosure](https://www.promptarmor.com/resources/microsoft-copilot-cowork-exfiltrates-files)). Treat any recurring agent-authored message workflow as a higher-priority audit target.

## Audit Checklist

For any agent that can write to a user-facing surface, answer four questions:
