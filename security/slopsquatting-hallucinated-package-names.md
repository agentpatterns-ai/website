...
- [Benign Skill Wording That Steers Package Hallucination (Neutral Prompting Attack)](neutral-prompting-hallucination-steering.md) — the active case: a third-party skill raises the hallucination rate instead of leaving the attacker to enumerate a fixed prior, and the registry existence check above measures no reduction against it
