...
- Query-string exfiltration to allowlisted domains: a prompt-injected fetch of a legitimate target still encodes user data in the URL, so pair it with a [public-web index gate](url-exfiltration-guard.md).
...
- Authenticated misuse: the allowlist decides whether a request reaches a destination, while a [scoped-credentials proxy](scoped-credentials-proxy.md) decides which credentials attach.
