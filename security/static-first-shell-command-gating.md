...
- Stereotyped destructive idioms rather than obfuscated ones. On CVE-derived snippets from Exploit-DB, plain regex beats the whole pipeline on F1 (81.52% against 67.98%), so the AST and rule bank are cost without benefit for that distribution ([Liu et al., 2026](https://arxiv.org/abs/2607.21642v2)).
