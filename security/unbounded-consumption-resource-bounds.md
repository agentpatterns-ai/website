...
- **Fixed thresholds without cost-velocity telemetry** — "100 calls/min" misses the *Continual Inconspicuous DoW* pattern (low and slow over hours), which is "difficult to distinguish from legitimate traffic patterns" ([arxiv:2508.19284](https://arxiv.org/html/2508.19284v1)). It also over-triggers on legitimate bursty workflows: a document-summarisation task involving file retrieval, chunking, three LLM calls, and storage will trip a tight bucket; "one rogue script blocks all the user's legitimate work, including the work they need to debug the rogue script" ([Pignati, 2026](https://medium.com/@alessandro.pignati/ai-agent-rate-limiting-is-broken-7eacc83a4129)). Tuple-keyed limits on `(user, repo, model)` plus rolling-average velocity beat fixed absolutes.
