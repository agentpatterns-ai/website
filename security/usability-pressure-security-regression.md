...
This is the in-context reward-hacking mechanism Pan et al. identified: models hack proxy objectives when the proxy is "under-specified and does not capture implicit constraints" ([Pan et al., 2024](https://arxiv.org/html/2402.06627v3)). UPAttack weaponizes the asymmetry without naming it.
...
**Make security explicit in the prompt.** A [Security Constitution](security-constitution-ai-code-gen.md) lists MUST/SHOULD security principles by CWE class and feeds them into the agent spec. The asymmetry collapses when the security side of the prompt is as load-bearing as the usability side.
...
**Before** — prompt expresses only usability:
...
**After** — prompt names the security invariant:
