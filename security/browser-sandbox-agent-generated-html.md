...
`default-src 'none'` collapses every fetch surface to "blocked." The two `'unsafe-inline'` exceptions re-enable inline scripts and styles the artifact ships with; `img-src data: blob:` permits in-page synthesised images without admitting any network origin. Once parsed, the policy is immutable: cross-browser testing on Chromium and Firefox confirms that JavaScript inside the iframe cannot remove, modify, or escape the `<meta>` tag, including via document replacement or `data:` URI navigation ([simonw/research: test-csp-iframe-escape](https://github.com/simonw/research/tree/main/test-csp-iframe-escape)).
