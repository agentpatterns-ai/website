...
Pass 1 — forward simulate the execution chain. The model walks through what the task does: which files the commands read, write, and execute. The goal is coverage. Over-granting is acceptable here; under-granting breaks the task and is harder to recover from.
