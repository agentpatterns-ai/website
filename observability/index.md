...
- [Programmatic Agent Session Export via `claude agents --json`](claude-agents-json-session-export.md) — Print live background sessions as a JSON array for status bars, dashboards, and tmux-resurrect integrations — the inventory primitive that pairs with OTEL `agent_id` for sub-session attribution
