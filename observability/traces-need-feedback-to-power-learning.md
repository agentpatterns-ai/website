...
The same trace can describe a 40-step success or a 40-step failure. Without a verdict, you cannot filter the trace store for failures worth turning into evals, compare good and bad trajectories on the same task, drive [incident-to-eval synthesis](../verification/incident-to-eval-synthesis.md) from production volume, or detect drift across the three improvement layers — model weights, harness scaffolding, retrieved context ([LangChain](https://www.langchain.com/blog/agent-observability-needs-feedback-to-power-learning)).
...
The article's deterministic example: Claude Code's leaked `userPromptKeywords.ts` regex scans prompts for frustration words like "wtf", "horrible", "awful" and emits the hit as a feedback signal ([PCWorld](https://www.pcworld.com/article/3104748/claude-code-is-scanning-your-messages-for-curse-words.html), [Blake Crosley analysis](https://blakecrosley.com/blog/claude-code-source-leak)). When a cheap rule captures a useful signal, you do not need a model call to label the trace.

## What the Platform Has to Do

Chase reduces the platform contract to three behaviours: store traces (trajectory, model and tool calls, metadata, timing, errors), store feedback attached to the run/trace/thread, and generate feedback (rules, online evaluators, sampling, annotation queues) ([LangChain](https://www.langchain.com/blog/agent-observability-needs-feedback-to-power-learning)).
...
- **Trace volume outpacing labelling capacity** — High-traffic agents accumulate traces faster than humans or judges can label them. Without sampling or filtering rules, the trace store becomes a graveyard of unlabeled trajectories.
