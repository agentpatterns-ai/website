...
Across 200 spans and 12 subagents, span nesting alone does not cheaply answer two questions: which spans belong to a given subagent, and which call chain caused this 429. Span hierarchy answers the first by tree traversal, but only inside a single Claude Code session. Once work crosses a boundary the instrumentation does not cover (shell-out, webhook, queue), lineage is gone.

The fix is a flat, propagated identifier that survives where the trace context ends.

## The propagation contract

Claude Code 2.1.139 ([changelog, May 11, 2026](https://code.claude.com/docs/en/changelog)) instruments subagents on two surfaces at once:
...
Both attributes matter. `agent_id` answers "all work by this subagent". `parent_agent_id` reconstructs the dispatch hierarchy from a flat query — "which subagent spawned this 429-emitting child" — without walking the span tree.
...
`agent.name` is the metric-safe partner. Dashboards aggregate by it to avoid cardinality explosions. Trace queries narrow to a specific incident via `agent_id`.
...
- Shell-out via the Bash tool: `curl -X POST https://api.example.com` produces a `claude_code.tool` span, but the request carries no `x-claude-code-agent-id` header — the service sees an anonymous request.
...
- Fire-and-forget queues: enqueueing discards the header, so the work runs untagged.
...
After — with `agent_id` and `parent_agent_id` on every span, the trace store answers both questions with a flat query:
