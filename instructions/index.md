...
- [Publishing Agent Instructions Outside the Repo (design.md)](publishing-agent-instructions-outside-the-repo.md) — A file read beside the code it governs leans on that code for meaning; published at a URL it needs named primitives and observable decisions in place of the missing examples
...
- [Constraint Preambles and the Gain Your Scanner Misses](constraint-preamble-before-generation.md) — A fixed block of output constraints cut named defect classes in all five models tested, and the two classes that moved most sit outside every scanner's rule set
...
- [Why an Encoded Rule Still Fails After a Passing Eval](encoded-rule-failure-diagnosis.md) — A rule that passes the eval and keeps failing in production is unclear, not loading, inexpressible in the available primitives, or in the wrong medium; only the first is wording
