...
The two benchmarks differ structurally. HumanEval relies on a single docstring. LiveCodeBench layers four largely independent surfaces: natural-language description, an explicit constraints block, sample I/O pairs with explanations, and an input/output format spec. Their summary: "robustness is not a fixed property of LLMs but is highly dependent on prompt structure" ([Akli et al., 2026](https://arxiv.org/html/2604.24712v1)).

Model scale does not rescue brittleness. API reasoning models (GPT-5-mini, Claude Sonnet 4) lose ~9.7% on US — comparable to the smallest open-source models tested ([Akli et al., 2026](https://arxiv.org/html/2604.24712v1)).
...
The same study identified 69 LiveCodeBench tasks where removing a constraint consistently improved Pass@1 across models. Three failure modes ([Akli et al., 2026](https://arxiv.org/html/2604.24712v1)):
...
- HumanEval has documented training-data contamination (8–18% overlap acknowledged in the paper). Treat absolute deltas as direction-of-effect evidence, not point estimates ([Akli et al., 2026](https://arxiv.org/html/2604.24712v1)).
