...
- Output flows into a Markdown-rendering surface — GitHub issue and PR comments, Slack, READMEs, docs sites, CI annotations. HTML appears as raw tags or is escaped.
- Token budget dominates — small models, free tiers, and batch pipelines pay the verbosity tax against a tight [context budget](../context-engineering/context-budget-allocation.md) without using the interactive layer.
...
The StableLearn rule of thumb: ask for HTML when the output needs repeated reading, review, comparison, or follow-up editing. For disposable notes, stay with Markdown ([StableLearn](https://stable-learn.com/en/claude-code-html-output/)).
...
A reviewer asks Claude Code for a summary of a 15-file refactor. The same task, two output-format requests:
