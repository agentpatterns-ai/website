...
- **Single-benchmark generalisation risk.** The +4.7pp gain comes from one benchmark (SWE-bench Verified) that is 46% Django; non-Django repos in the eval had 1–2 instances each, so the headline number's robustness to a more diverse evaluation is untested ([Shepard & Albrecht, 2026 §9](https://arxiv.org/html/2606.20512)).
