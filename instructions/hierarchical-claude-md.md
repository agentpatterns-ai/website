...
**Local (`./CLAUDE.local.md`)**: Personal project-specific preferences, not checked into version control. Claude Code automatically adds this file to `.gitignore` [unverified]. Use it for sandbox URLs, personal test data, or per-machine settings.
...
Every token in a CLAUDE.md file is loaded into context before task work begins [unverified]. Bloated files consume context budget the agent needs for the actual task.
...
Directory-level files solve this cleanly: each service documents its own conventions in its own file, and the agent automatically loads the relevant file based on which directory it is working in [unverified].
...
- Claude Code automatically adds CLAUDE.local.md to .gitignore [unverified]
- Every token in a CLAUDE.md file is loaded into context before any task work begins [unverified]
- The agent automatically loads the directory-level CLAUDE.md file based on which directory it is working in [unverified]
