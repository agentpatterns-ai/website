...
JetBrains coined the term **Shadow Tech Debt** `[unverified]` to describe debt that is invisible, diffuse, and rapidly compounded when agents run autonomously without structural understanding of the codebase.

## What It Looks Like

An agent fixes a bug. The change passes tests. But the agent didn’t read ADRs, didn’t know naming conventions, replicated a suboptimal pattern, or added an abstraction that conflicts with the design philosophy. One such PR is invisible. Ten per day compounds into structural incoherence.
...
**Agents amplify existing patterns.** Suboptimal approaches propagate when agents replicate whatever exists in the repository ([Lavaee](https://alexlavaee.me/blog/openai-agent-first-codebase-learnings/)).

**Review burden migrates, not disappears.** Teams with high AI adoption merged 98% more PRs, but review time ballooned 91% and PR size grew 154% `[unverified]` ([Faros AI](https://www.faros.ai/blog/ai-software-engineering); [Osmani](https://addyo.substack.com/p/the-80-problem-in-agentic-coding)).
...
When agents commit to shared repositories without review gates, Shadow Tech Debt accumulates at machine speed. JetBrains AIR concluded that complex codebases aren’t yet ready for pure agentic coding `[unverified]`, recommending human-in-the-loop orchestration with mandatory review gates.

## Mitigation Stack

| Step | Effort | Action |
|------|--------|--------|
| 1 | Low | **Machine-readable context files** — AGENTS.md at the repo root `[unverified]`; CLAUDE.md for Claude Code sessions. Scoped files (`docs/CLAUDE.md`) for monorepos. |
| 2 | Medium | **Deterministic enforcement** — linters and structural tests for module boundaries, naming, and duplication. Fowler/Boeckeler call this “[rigor relocation](../human/rigor-relocation.md)” ([source](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html)). |
| 3 | Medium | **Review gates** — never let autonomous agents merge without human review on shared repositories. |
| 4 | High | **Garbage-collection agents** — background agents that scan for architectural inconsistencies ([Fowler/Boeckeler](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html); [Lavaee](https://alexlavaee.me/blog/openai-agent-first-codebase-learnings/)). Requires step 1. |
