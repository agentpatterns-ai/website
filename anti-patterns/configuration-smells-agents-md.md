...
- **Context Bloat** — `javascript-obfuscator`'s CLAUDE.md was 1,477 lines across 27 sections, with product documentation that belonged in a separate `docs/` directory.
- **Skill Leakage** — `quickemu-project` carried detailed *"Adding a new OS"* instructions used only for one task type.
- **Conflicting Instructions** — `inkline/inkline` told the agent components live in `packages/ui/components` *and* in `packages/components`.
...
- **Projects already on a strict pointer-map regime** — see [AGENTS.md as a Table of Contents](../instructions/agents-md-as-table-of-contents.md); the discipline is already enforced.
