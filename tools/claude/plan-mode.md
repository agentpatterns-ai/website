...
Learn it hands-on with the [Plan Mode and Plan-First guided lesson](https://learn.agentpatterns.ai/harness-engineering/plan-mode-and-plan-first/), which includes quizzes.

Plan Mode is a permission mode in Claude Code that blocks all file writes and state-changing shell commands. The agent can read files, search code, and analyze the codebase, but cannot modify anything. This separates understanding from implementation, so you catch wrong approaches before they produce wrong code.

## Why it works

Fixing a plan costs minutes. Fixing an implementation costs context, tokens, and git reverts. Plan Mode makes the agent's understanding of the task visible. If it targets the wrong files, misreads the architecture, or picks an approach that conflicts with existing patterns, you see that in the plan, not in a broken diff.

Read-only constraints force the agent to ask questions (via `AskUserQuestion`) and propose steps rather than run them. You review, correct, and approve before switching to implementation.
...
The status bar shows that plan mode is active while it is in effect.
...
1. Explore. The agent reads the codebase and maps the relevant files, functions, and dependencies.
2. Plan. The agent proposes a step-by-step approach. Press Ctrl+G to open the plan in your text editor and edit it directly.
3. Implement. Switch to Normal Mode. The agent works against the reviewed plan.
4. Verify. Run tests, review the diff, and check for regressions.

When you accept a plan, Claude names the session from the plan content, which helps when you resume later.
...
| `useAutoModeDuringPlan` | Use [auto mode](auto-mode.md) semantics during plan mode | `true` |
...
- [The Plan-First Loop](../../workflows/plan-first-loop.md) — The general pattern of designing before coding, independent of tool-specific features
...
- [Pre-Execution Codebase Exploration](../../workflows/pre-execution-codebase-exploration.md) — Structured exploration of a codebase before making changes
- [Human-in-the-Loop Placement](../../workflows/human-in-the-loop.md) — Where to place human approval gates in agent pipelines; Plan Mode is one mechanism for inserting a review checkpoint before execution
