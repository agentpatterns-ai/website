...
This mirrors feature flag best practices in production systems: progressive rollout without breaking existing workflows [unverified].

## Performance and Model Control

| Variable | Effect | Default |
|----------|--------|---------|
| `CLAUDE_CODE_EFFORT_LEVEL` | Adaptive reasoning level: `low`, `medium`, `high` | `medium` for Max/Team ([model config docs](https://code.claude.com/docs/en/model-config)) |
| `MAX_THINKING_TOKENS` | Extended thinking token budget (used when adaptive thinking is disabled) | Model-dependent |
| `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | Cap on output tokens per response | 32,000 ([settings docs](https://code.claude.com/docs/en/settings)) |
| `CLAUDE_CODE_SUBAGENT_MODEL` | Force a specific model for sub-agent operations | Inherits main model tier [unverified] |
...
| `ENABLE_CLAUDE_CODE_SM_COMPACT` | Enable session memory compaction [unverified] | Disabled |
...
| `CLAUDE_CODE_BASH_MAINTAIN_PROJECT_WORKING_DIR` | Reset to project directory after each bash command | Not set ([settings docs](https://code.claude.com/docs/en/settings)) |
...
| `BASH_MAX_OUTPUT_LENGTH` | Maximum bash output characters (middle-truncated) | [unverified] ([settings docs](https://code.claude.com/docs/en/settings)) |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OpenTelemetry exporter endpoint for traces, metrics, and logs [unverified] | Not set |
