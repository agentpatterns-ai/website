...
- **Token cost scales with agent count.** A run that spawns dozens of agents can use far more tokens than working the same task in conversation. Every agent uses your session's model unless the script routes a stage elsewhere — check `/model` before a large run.
...
- **Research preview.** The feature and its limits may change; gate dependent automation behind a version check and confirm against the [canonical docs](https://code.claude.com/docs/en/workflows).
