...
Claude researches the codebase, identifies 12 modules that need changes, presents a plan, then spawns 12 agents — each in its own worktree editing a separate module. Each agent commits to its own branch, and Claude collects the results when all agents finish.
...
- Units are not truly independent — if the planned slices touch shared files or depend on one another's output, the parallel agents race on merge, and you pay review cost on 12+ PRs instead of one.
...
- The change is exploratory — when the right shape of the refactor is not known yet, a plan decomposed into 5 to 30 units locks in a premature design that later units cannot cheaply revise.
...
- Quota ceilings hit fast — practitioners report a thread-limit wall above ~8 concurrent agents and quota burn that scales linearly with fan-out ([findskill.ai](https://findskill.ai/blog/claude-code-10-parallel-agents-week-1/)).
