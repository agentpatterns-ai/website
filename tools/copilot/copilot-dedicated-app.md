...
- **Concurrency > 1.** Multiple parallel sessions need a scannable inbox more than a chat panel; the home view treats each session as a row. Below one parallel session, the [Manager Surface gain](../../agent-design/editor-manager-surface-separation.md#why-it-works) does not pay back its switch cost.
- **Multi-repo dispatch.** An inbox aggregating issues and PRs across "connected repositories" ([GitHub, 2026-05-14](https://github.blog/changelog/2026-05-14-github-copilot-app-is-now-available-in-technical-preview/)) beats opening N IDE windows.
- **Session-as-unit work.** When the developer reviews diffs and approves merges more than they type, the editor's cursor-and-buffer model is overhead. A separate process can optimise for sessions, not files.
