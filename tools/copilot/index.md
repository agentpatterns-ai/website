...
- [Copilot App Diff, Terminal, and Browser Panels](copilot-app-diff-terminal-browser.md) — The three working surfaces beside the chat pane: two diffs with different exits, a terminal you drive against a session worktree, and a browser panel whose governance is published for VS Code and absent here
...
- [Bounding an Embedded Copilot SDK Agent's Tool Set](copilot-sdk-tool-scoping.md) — The tool allowlist and the permission hook are independent controls, and a handoff orchestrator supplies transfer tools at invocation that the session config never named
