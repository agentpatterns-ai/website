...
- Illusion of explanatory depth (IOED). In a three-arm N=102 study, participants who received GPT explanations showed the largest gap between predicted and actual explanation quality. They produced less accurate, shorter, lower-vocabulary explanations than the no-GPT arm ([Sciety preprint](https://sciety.org/articles/activity/10.31234/osf.io/8psgf_v1)). Fluent agent prose triggers cognitive ease, and the feeling of understanding runs ahead of understanding.
- Post-hoc rationalization. The agent's stated reason for "why YAML" need not be the computation that produced the choice. The faithfulness literature documents LLM justifications that are persuasive and causally unrelated to the underlying answer ([arXiv 2603.01190](https://arxiv.org/html/2603.01190)).
- Performance is not learning. In AI-assisted programming education, students outperformed controls on the task but showed no pre-post knowledge gain — completion, not transfer ([ScienceDirect 2025](https://www.sciencedirect.com/science/article/pii/S2666920X25001778)).
- Scaffolding threshold. Self-explanation effects shrink for less-prepared learners under high working-memory load ([Bisra et al. 2018](https://link.springer.com/article/10.1007/s10648-018-9434-x)). A non-engineer self-explaining unfamiliar tooling while still learning git is below threshold.
...
Hannah Stulberg names the pattern verbatim: "I have a feature index in the repo as a YAML file. If someone didn't understand why this was a YAML file, I would ask Claude to explain the benefits" (Aakash Gupta podcast, 1:17:16, [transcript](https://www.aakashg.com/hannah-stulberg-podcast/)). The companion write-up formalizes it: "when you encounter a file you do not understand — explain why this is a YAML file and not markdown" ([source](https://www.news.aakashg.com/p/claude-code-team-os)).

The escalation prompt is the one upgrade over a bare "why?": "what are 2–3 alternatives and when would each win?" This forces the agent onto a tradeoff surface that is harder to confabulate coherently, and it returns falsifiable conditions the user can test later.

## Scope: when to ask

Only for decisions the user will curate long-term:

- Tool and format choices (YAML vs Markdown, JSON vs TOML), often part of a [cross-functional artifact](cross-functional-artifacts.md) the team shares.
- [Folder layout and ownership boundaries](functional-folder-taxonomy.md).
- `CLAUDE.md` placement and hierarchy.
- Plan-mode gates and naming conventions.

Not for every output. If the user can already name the alternative and one reason it is worse, skip the loop.
