...
- Heterogeneous call types. Support escalations, renewal negotiations, and in-depth technical calls do not map cleanly onto six discovery-shaped sections. Forcing them in produces empty `## Feature Requests` blocks that pollute aggregation; fork the schema by call kind instead.
