...
A framework speeds up your work when its primitives match your task and fights you when they do not. Five axes carry most of the decision:
...
Code-first Python framework — Python 3.11+, Apache 2.0 ([ADK README](https://github.com/google/adk-python)). The 2.0 release builds on two primitives: a graph-based Workflow Runtime with routing, fan-out/fan-in, loops, retry, state, human-in-the-loop, and nested workflows, and a Task API for structured agent-to-agent delegation. Workflows are durable: from Python 1.16, a resumable workflow persists checkpoints via `DatabaseSessionService` (SQLite or Cloud SQL) and resumes from an Invocation ID after interruption ([ADK Resume Agents](https://google.github.io/adk-docs/runtime/resume/)).

Fits when: you run multi-agent workflows with explicit state transitions, long-running processes that need pause and resume across days, or Google Cloud or Gemini-first stacks.

Skip when: a single-agent short task where a `while` loop suffices, or non-Python stacks.

### Agno

Python SDK positioned as "build, run, and manage agent platforms" ([Agno README](https://github.com/agno-agi/agno)). The runtime ships 50+ HTTP/SSE/WebSocket endpoints, OpenTelemetry tracing, JWT-based RBAC for multi-tenant isolation, cron scheduling, and pre-built Slack/Telegram/WhatsApp interfaces. Third-party benchmarks report about 2 μs agent instantiation and about 3.75 KiB per agent — roughly four orders of magnitude lighter than LangGraph ([Analytics Vidhya, 2025](https://www.analyticsvidhya.com/blog/2025/03/agno-framework/)). The micro-overhead matters only when you instantiate thousands of agents per request.

Fits when: production agent platforms need multi-tenant isolation, RBAC, and observability out of the box.

Skip when: a single embedded agent inside a larger app — a control plane is overkill.

### Browser Use

Python (3.11+) agent that limits the action space to a Playwright-controlled browser with a DOM-aware action vocabulary ([Browser Use README](https://github.com/browser-use/browser-use)). Benchmarked on 100 real-world browser tasks ([browser-use/benchmark](https://github.com/browser-use/benchmark)). DOM-based actions hold up better on standard layouts than coordinate-based computer-use models, which "don't yet work reliably enough in production" ([Helicone: Browser Use vs Computer Use vs Operator](https://www.helicone.ai/blog/browser-use-vs-computer-use-vs-operator)).
...
Open-source desktop multi-agent application built on CAMEL-AI, packaged as a TypeScript/Electron app ([Eigent README](https://github.com/eigent-ai/eigent)). Local model integration (vLLM, Ollama, LM Studio), SSO/RBAC, MCP integration, and fully standalone deployment with no cloud account. The unit of distribution is an installed desktop app, not a library.

Fits when: you build end-user multi-agent productivity tooling, or on-premise local deployment is a hard requirement (regulated industries, air-gapped environments).

Skip when: you build an agent into another application — Eigent is the application, not a library.
...
Memory layer, not a runtime — Python and Node SDKs ([mem0 README](https://github.com/mem0ai/mem0)). It composes over any agent harness: you keep your loop and tools, and mem0 handles extraction, storage, and retrieval. The April 2026 algorithm uses single-pass ADD-only extraction with entity linking and multi-signal retrieval (semantic plus BM25 plus entity), reporting 92.5 on LoCoMo and 94.4 on LongMemEval at under about 7,000 tokens per query ([Mem0 research](https://mem0.ai/research)). The trade-off is real: an independent comparison measured mem0 at about 67% against full-context at about 73% — a roughly 6% accuracy gap as the price for token efficiency ([MindStudio: Mem0 vs OpenAI built-in memory](https://www.mindstudio.ai/blog/agent-memory-infrastructure-mem0-vs-openai)).

Fits when: you have a working agent and want cross-session memory without rewriting, or you run recall-heavy workloads with relaxed accuracy needs (personalization, support history).

Skip when: memory accuracy is load-bearing (medical, legal, compliance) — full-context with retrieval over authoritative sources beats lossy summarization; or the agent is single-session.
...
- Non-Python primary stack — Agno, ADK, and Browser Use are Python-first; mem0 and Letta ship Node SDKs, but most of the six force the language choice. A Go or Rust team that adopts them stands up a Python sidecar service, which complicates ops more than it simplifies agent code.
- Computer-control beyond the browser — Browser Use is browser-only, and Eigent is desktop-packaged but Electron-bound. OS-level automation (native apps, SSH workflows, system-wide tasks) needs a different harness shape, typically Anthropic's Computer Use or local OS scripting.
...
- High-stakes accuracy with mem0 — the roughly 6% accuracy gap is acceptable for personalization chat, not for compliance-critical recall ([MindStudio](https://www.mindstudio.ai/blog/agent-memory-infrastructure-mem0-vs-openai)).
...
Compare a customer-support agent that handles cross-session conversation history for thousands of users: the horizon is long, the memory model is server-stateful per user, and the deployment is a control plane. Letta is now the closest fit; mem0 is the lighter alternative if accuracy can absorb the gap.
